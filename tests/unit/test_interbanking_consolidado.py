...
DESDE, HASTA = date(2026, 8, 14), date(2026, 8, 18)
...
def _cuenta(**kw):
    base = {
        ...
        "saldo_inicio": None, "saldo_cierre": None, "dias": None,
        "f_ini": None, "f_fin": None,
        "s_saldo": None, "s_fecha": None,
        "proyectado_24hs": None, "proyectado_48hs": None,
    }
    return {**base, **kw}
...
def test_con_extracto_manda_el_extracto(sin_base):
    sin_base([_cuenta(saldo_inicio=100, saldo_cierre=150, dias=2,
                      f_ini=DESDE, f_fin=HASTA, s_saldo=150, s_fecha=HASTA)])
    c = bancos.consolidado("x@y", DESDE, HASTA)["bancos"][0]["cuentas"][0]
...
def test_la_cuenta_QUIETA_sale_del_saldo_y_no_con_guion(sin_base):
    ...
    sin_base([_cuenta(s_saldo=4321.55, s_fecha=HASTA)])
    r = bancos.consolidado("x@y", DESDE, HASTA)
    ...
    # Y entra al total, que antes quedaba corto por abajo.
    assert r["totales"]["ARS"]["cierre"] == 4321.55
...
def test_sin_extracto_la_variacion_es_None_no_cero(sin_base):
    ...
    sin_base([_cuenta(s_saldo=999, s_fecha=HASTA)])
    c = bancos.consolidado("x@y", DESDE, HASTA)["bancos"][0]["cuentas"][0]
...
def test_sin_ninguna_fuente_sigue_en_guion_y_cuenta_como_sin_datos(sin_base):
    ...
    assert r["totales"]["ARS"]["cuentas"] == 0, "una cuenta sin dato no aporta al total"
...
def test_si_las_dos_fuentes_no_coinciden_se_publica_la_diferencia(sin_base):
    ...
    sin_base([_cuenta(saldo_inicio=100, saldo_cierre=150, dias=2,
                      f_ini=DESDE, f_fin=HASTA, s_saldo=140, s_fecha=HASTA)])
    c = bancos.consolidado("x@y", DESDE, HASTA)["bancos"][0]["cuentas"][0]
...
def test_el_cbu_no_se_filtra_ni_por_esta_via(sin_base):
    ...
    sin_base([_cuenta(s_saldo=1, s_fecha=HASTA,
