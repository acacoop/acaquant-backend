...
# ── NEGOCIO: LOS BANCOS (§0.dk) ──────────────────────────────────────────────

def test_banco_no_cierra_lee_lo_que_la_tab_ya_calcula_y_ve_el_arrastre(monkeypatch):
    """Martes simulado: el Banco X no cierra el lunes por 1,2 M (la tab lo
    muestra si alguien la abre), el mismo banco lleva tres días sin conciliar
    contra el mayor (la tab NO lo muestra: juzga cada día aislado), el banco
    informa dos cierres distintos, y un pendiente confirmado hace 8 hábiles
    sigue abierto."""
    from datetime import date

    from agente import fuentes, reloj
    from agente.detectores import negocio

    monkeypatch.setattr(reloj, "hhmm", lambda a=None: "12:05")
    ext = [
        {"cuenta_id": 1, "cuenta": "Banco X CC ARS", "fecha": date(2026, 9, 1), "cierra": False,
         "diferencia": 1_200_000.0, "saldo_cierre": 10.0, "saldo_dia": 10.0,
         "numero_extracto": "E-77"},
        {"cuenta_id": 2, "cuenta": "Banco Y CC USD", "fecha": date(2026, 9, 1), "cierra": True,
         "diferencia": 0.0, "saldo_cierre": 500.0, "saldo_dia": 480.0, "numero_extracto": "E-9"},
    ]
    monkeypatch.setattr(fuentes, "bancos_extractos", lambda habiles=2: ext)

    def fila(cid, cuenta, concilia, dif):
        return {"id": cid, "etiqueta": cuenta, "banco_nombre": "", "numero": "", "moneda": "",
                "concilia": concilia, "diferencia": dif, "dif_sin_gastos": dif,
                "cierre_banco": 100.0, "saldo_final": 100.0 - (dif or 0),
                "movimientos_mayor": 3, "gastos": 0.0}
    d1, d2, d3 = date(2026, 9, 1), date(2026, 8, 31), date(2026, 8, 28)
    tabs = {d1: [fila(1, "Banco X CC ARS", False, 50.0), fila(2, "Banco Y CC USD", False, 7.0)],
            d2: [fila(1, "Banco X CC ARS", False, 50.0), fila(2, "Banco Y CC USD", True, 0.0)],
            d3: [fila(1, "Banco X CC ARS", False, 50.0), fila(2, "Banco Y CC USD", True, 0.0)]}
    monkeypatch.setattr(fuentes, "bancos_tableros", lambda dias=3: tabs)
    monkeypatch.setattr(fuentes, "bancos_pendientes", lambda: [
        {"id": 41, "cuenta": "Banco X CC ARS", "fecha": "2026-08-20", "accion": "falta_en_el_mayor",
         "descripcion": "N/C REVERSO", "importe": -50.0, "resuelto": False, "dias_habiles": 8},
        {"id": 42, "cuenta": "Banco X CC ARS", "fecha": "2026-09-01", "accion": "falta_en_el_mayor",
         "descripcion": "nuevo", "importe": -1.0, "resuelto": False, "dias_habiles": 1},
    ])

    por = {(h.sujeto, h.regla): h
           for h in negocio.banco_no_cierra({"pendiente_dias": 5, "arrastre_dias": 3})}
    x, y = "Banco X CC ARS", "Banco Y CC USD"
    assert por[(x, "extracto_no_cierra")].severidad == "alta"
    assert por[(x, "extracto_no_cierra")].evidencia["items"] == ["2026-09-01"]
    assert (y, "extracto_no_cierra") not in por
    assert por[(y, "saldo_discrepante")].evidencia["diferencia"] == 20.0
    assert (x, "saldo_discrepante") not in por
    # X: tres días seguidos → arrastre, y NO además el diario. Y: solo hoy → diario.
    assert por[(x, "descuadre_arrastrado")].evidencia["dias_seguidos"] == 3
    assert (x, "banco_vs_mayor") not in por
    assert por[(y, "banco_vs_mayor")].severidad == "media"
    assert por[(x, "pendiente_viejo")].evidencia["items"] == ["41"]
    for h in por.values():
        assert h.que_hacer and "12:05" in h.problema


def test_banco_no_cierra_ciego_levanta_y_el_dominio_negocio_existe(monkeypatch):
    from agente import fuentes
    from agente.detectores import negocio

    for f in ("bancos_extractos", "bancos_tableros", "bancos_pendientes"):
        monkeypatch.setattr(fuentes, f, lambda *a, **k: None)
    with pytest.raises(tipos.SinDatos):
        negocio.banco_no_cierra({})
    # Una sola fuente viva: las otras no cierran nada, pero esa corre.
    monkeypatch.setattr(fuentes, "bancos_pendientes", lambda: [])
    assert negocio.banco_no_cierra({}) == []

    h = catalogo.HABILIDADES["banco_no_cierra"]
    assert h.dominio == "NEGOCIO" and "NEGOCIO" in tipos.DOMINIOS
    assert h.ventana == "habil" and not h.arreglos
    assert h.umbrales == {"pendiente_dias": 5, "arrastre_dias": 3}
