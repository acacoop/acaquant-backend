...
-- CHEQUES EMITIDOS (tab CHEQUES, lado derecho). Carga MANUAL del back office —
-- mismo patrón que SENEBIS: no viene de Aunesa, lo escribe una persona. El lado
-- IZQUIERDO de la tab (recibidos) NO se persiste: son los movimientos e-cheq del
-- día que ya manda Aunesa, se sirven live igual que el resto de la tesorería.
...
CREATE TABLE IF NOT EXISTS operaciones.tesoreria_cheques (
    ...
    unidad          text NOT NULL DEFAULT 'ARS',  -- moneda del banco elegido
...
CREATE INDEX IF NOT EXISTS ix_tesoreria_cheques_estado
    ON operaciones.tesoreria_cheques (estado);
