...
# ── EL CIERRE CONTRA SU PROPIA HISTORIA (§0.dj) ────────────────────────────
#
# Lo que miraba `jobs/guardrails.py` con umbrales que nacieron en `None` y
# nunca se calibraron. Acá no hay umbral que calibrar: la normalidad se MIDE
# sobre las últimas fechas, igual que `tabla_quieta` mide la cadencia.
def aum_serie(fechas: int = 20) -> list[tuple] | None:
    """`[(fecha, total), …]` de las últimas N fechas del writer diario, la más
    nueva primero. Scopeado por fecha (N fechas puntuales, no un scan)."""
    def _leer():
        with get_pool().connection() as conn, conn.cursor() as cur:
            cur.execute("SELECT DISTINCT fecha FROM portafolio.tenencia "
                        "WHERE aum = 'si' ORDER BY fecha DESC LIMIT %s", (int(fechas),))
            fs = [r[0] for r in cur.fetchall()]
            if not fs:
                return []
            cur.execute("SELECT fecha, sum(valuacion) FROM portafolio.tenencia "
                        "WHERE aum = 'si' AND fecha = ANY(%s) "
                        "GROUP BY fecha ORDER BY fecha DESC", (fs,))
            return [(r[0], float(r[1] or 0)) for r in cur.fetchall()]
    return _una_vez("aum_serie", _leer)


def cierres(fechas: int = 20) -> dict | None:
    """El cierre por bono de las últimas N fechas de `snapshots_cierre_hist`.

    `{"fechas": [la más nueva primero], "precios": {fecha: {ticker: precio}},
      "filas_hoy": [{ticker_corto, ultimo_precio}, …]}` — `filas_hoy` trae la
    fila cruda de la fecha más nueva, con nulls y ceros, que es lo que
    `cierre_invalido` necesita ver."""
    def _leer():
        with get_pool().connection() as conn, conn.cursor() as cur:
            cur.execute("SELECT DISTINCT fecha FROM mercado.snapshots_cierre_hist "
                        "ORDER BY fecha DESC LIMIT %s", (int(fechas),))
            fs = [r[0] for r in cur.fetchall()]
            if not fs:
                return {"fechas": [], "precios": {}, "filas_hoy": []}
            cur.execute("SELECT fecha, ticker_corto, ultimo_precio "
                        "FROM mercado.snapshots_cierre_hist WHERE fecha = ANY(%s)", (fs,))
            precios: dict = {f: {} for f in fs}
            filas_hoy = []
            for fecha, tk, px in cur.fetchall():
                if fecha == fs[0]:
                    filas_hoy.append({"ticker_corto": tk,
                                      "ultimo_precio": float(px) if px is not None else None})
                if tk and px is not None and float(px) > 0:
                    precios[fecha][tk] = float(px)
            return {"fechas": fs, "precios": precios, "filas_hoy": filas_hoy}
    return _una_vez("cierres", _leer)


def emisores_estado() -> dict | None:
    """`{"sin_industria": [...], "contradicciones": [...]}` del catálogo de
    emisores, por el service que ya usa Manager (`api/services/emisores`)."""
    def _leer():
        from api.services import emisores
        return {"sin_industria": list(emisores.sin_clasificar() or []),
                "contradicciones": list(emisores.contradicciones() or [])}
    return _una_vez("emisores_estado", _leer)
