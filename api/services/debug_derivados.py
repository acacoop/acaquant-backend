"""api/services/debug_derivados.py — debugs paso-a-paso de renta fija/derivados.

Servicio PURO (sin FastAPI) para el panel Manager → Validaciones. Antes estos
tres pipelines quant vivían INLINE en api/routers/manager/checks.py (violación
de la regla routers = thin plumbing); acá siguen el mismo patrón que
debug_curva.py. Errores de input: LookupError (no existe) / ValueError
(no aplica) — el router los mapea a 404/400.
"""
...
def debug_soberano(ticker_corto: str) -> dict:
    ...
    if not inst:
        raise LookupError(f"No existe ticker_corto={ticker_corto!r} en mercado.curvas")
