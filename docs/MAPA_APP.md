...
| **Control Títulos Negativos** | Títulos con **nominales < 0**, en **DOS tableros al 50%**: izq **T0** (liquidada a HOY = lo que está en custodia ahora → descubierto REAL) · der **T1** (liquidada a MAÑANA con lo concertado hoy → el que se viene). Columnas: CUENTA · TICKER · NOMINALES. Poll 20s. Cada lado con su contador de títulos y cuentas + **antigüedad del dato** en la barra | `/titulos-negativos` | `incluir_todo` (def **no** — MONEDAS y DERIVADOS quedan afuera: en los dos el negativo es normal, descubierto bancario / posición vendida); buscador `q` client-side por ticker/cuenta, aplica a los dos lados | Ninguna (solo lectura) |
