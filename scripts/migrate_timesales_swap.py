...
def mode_swap(force: bool = False) -> int:
    """Hace el swap completo:
    1. Crea TimeSales_ts como TS Collection.
    2. Copia toda la data en batches.
    3. Valida count match.
    4. Renombra TimeSales → TimeSales_old, TimeSales_ts → TimeSales.
    """
    client = get_mongo_client()
    db = client[DB_NAME]
    ...
    # Limpiar TMP previo si --force.
    if _coll_info(db, TMP):
        if not force:
            logger.error("%s.%s YA existe (probablemente intento previo). "
                         "Corré con --force para resetearla.", DB_NAME, TMP)
            return 1
        logger.info("--force: borrando %s.%s residual.", DB_NAME, TMP)
        db.drop_collection(TMP)

    # ── Paso 1: crear TS Collection ──
    logger.info("[1/4] Creando %s.%s como TS Collection (timeField=%s, "
                "metaField=%s, granularity=%s)…",
                DB_NAME, TMP,
                TIMESERIES_OPTIONS["timeField"],
                TIMESERIES_OPTIONS["metaField"],
                TIMESERIES_OPTIONS["granularity"])
    db.create_collection(TMP, timeseries=TIMESERIES_OPTIONS)

    # ── Paso 2: copiar data ──
    src = db[SRC]
    dst = db[TMP]
    total = src.count_documents({})
    logger.info("[2/4] Copiando %s docs de %s → %s (batches de %d)…",
                _fmt_size(total), SRC, TMP, COPY_BATCH)
    ...
    for doc in src.find({}, sort=[("timestamp", 1)]):
        ...
        if len(batch) >= COPY_BATCH:
            ...
            if pct >= last_progress + 5:
                ...
                eta = (total - copied) / max(rate, 1)
                logger.info("    %s/%s (%d%%) · %.0f docs/s · ETA %.0fs",
                            _fmt_size(copied), _fmt_size(total), pct, rate, eta)
    if batch:
        dst.insert_many(batch, ordered=False)
        copied += len(batch)
    ...
    # ── Paso 3: validar count match ──
    logger.info("[3/4] Validando count match…")
    n_src = src.count_documents({})
    n_dst = dst.count_documents({})
    if n_src != n_dst:
        logger.error("    [!! ] count MISMATCH: src=%d dst=%d (diff %+d). "
                     "ABORTANDO el swap.", n_src, n_dst, n_dst - n_src)
        logger.error("    %s.%s queda creada con la data parcial — borrarla "
                     "manualmente o correr --force en el próximo intento.",
                     DB_NAME, TMP)
        return 2
    logger.info("    [OK] count match: %s docs en ambas.", _fmt_size(n_src))

    # ── Paso 4: rename swap ──
    logger.info("[4/4] Rename swap: %s → %s y %s → %s",
                SRC, BACKUP, TMP, SRC)
    # rename usa adminCommand(renameCollection) con el namespace completo.
    admin = client.admin
    admin.command({
    ...
    admin.command({
        "renameCollection": f"{DB_NAME}.{TMP}",
        "to":               f"{DB_NAME}.{SRC}",
    })
    ...
    if not _is_ts(final_info):
        logger.error("    [!! ] %s.%s POST-swap NO es TS — algo se rompió.",
                     DB_NAME, SRC)
        return 3
