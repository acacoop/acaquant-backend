...
from dashboard.repos.manager import TRACES, fetch_latest
...
def _run_bg_aum_multi(fechas: list[str], key: str, extra_env: dict | None = None):
    ...
    script_path = os.path.join(PROJECT_ROOT, "jobs/aum_backfill.py")
...
def _status_live_rows(en_rueda: bool) -> list[dict]:
    ...
    with ThreadPoolExecutor(max_workers=len(_STATUS_LIVE)) as ex:
        docs = list(ex.map(
            lambda s: fetch_latest(s[0], s[1], s[2], {s[2]: {"$exists": True}}),
            _STATUS_LIVE,
        ))
...
def _status_periodico_rows() -> list[dict]:
    ...
    with ThreadPoolExecutor(max_workers=len(_STATUS_PERIODICO)) as ex:
        docs = list(ex.map(
            lambda s: fetch_latest(s[0], s[1], s[2],
    ...
    with st.expander("Match Contrapartes — vincular cuentas Aunesa ↔ MongoDB"):
        ...
        if st.button("▶ Ejecutar (modifica Mongo)", key="v_mc_btn"):
            with st.spinner("Autenticando y buscando matches..."):
                buf = io.StringIO()
                try:
                    from scripts.test_match_contrapartes import main as mc_main
...
def _latencia_trace():
    ...
    with col1:
        vista_sel = st.selectbox("Vista", list(TRACES.keys()), key="trace_vista")
    with col2:
        st.write("")
