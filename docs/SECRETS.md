...
| `DEEPSEEK_API_KEY` | LLM del sistema para tareas de MERCADO — copiloto, triage, research (ruteo en `core/llm.py`). Cuenta prepaga, saldo chico: si se filtra, el daño máximo es quemar el saldo. Sin ella esas features degradan (todo lo demás sigue). ⚠ Este proveedor SÍ puede entrenar con lo que se le manda → jamás datos del negocio. | platform.deepseek.com → API Keys → regenerar → `.env` (los crons la toman solos; restart API cuando la use la API). |
| `OPENAI_API_KEY` | LLM del **asistente de negocio** (tarea `asistente_negocio`, la única que ve números del negocio — sin identidades: las tacha la aduana `core/pii_gateway.py`). Se eligió por su compromiso de NO entrenar con datos de API + borrado a 30 días (decisión user 2026-07-21). **Sin ella el asistente se apaga y NO cae a DeepSeek** (fail-closed de ruteo). Verificar con `python -m scripts.smoke_asistente --ruteo`. | platform.openai.com → API keys → regenerar → `.env` → restart API. Settings → Organization → Data controls: sharing todo en **Disabled**, API call logging **Disabled**, audit logging **Enabled**. |
