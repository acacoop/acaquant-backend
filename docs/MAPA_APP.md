...
| **Interbanking** | Los bancos de ACA, para CONCILIAR. **DOS sub-tabs.** **CONSOLIDADO BANCOS** (default): UNA fila por cuenta agrupadas bajo el **nombre del banco como título**, con **SALDO AL INICIO** (apertura del primer día con extracto del rango) y **SALDO AL CIERRE** (cierre del último) + variación; subtotal por banco y total global, **por moneda y nunca mezclados**. Lista TODAS las cuentas activas. **El saldo tiene DOS fuentes y la fila declara cuál** (`fuente`): el EXTRACTO si lo hay (apertura del primer día con extracto, cierre del último) y, si la cuenta no se movió en el rango y por lo tanto no tiene extracto, el **SALDO que informa el banco** (`bancos.saldos`, rotulado «saldo» en la celda) — antes esas iban con «—» porque el extracto solo devuelve los días CON actividad, y con la ventana corta el agujero era grande. Nunca se suman las dos: si el banco informa un cierre de extracto distinto del saldo del día, se marca **≠** con la diferencia (hallazgo de conciliación, no un número a elegir). Sin ninguna de las dos sigue «—» y la barra dice cuántas están así. Columnas **Proy. 24hs / 48hs** con los saldos proyectados que informa el banco (no se subtotalizan: solo los informan algunas cuentas). **DETALLE POR CUENTA**: una cuenta repartida **1/3 · 2/3** — izquierda **EXTRACTO** (apertura · créditos · débitos · cierre), derecha **MOVIMIENTOS** con 8 columnas (FECHA · DESCRIPCIÓN · CONCEPTO · COD OP · COD OP BCO · COMPROBANTE · SUCURSAL · IMPORTE), con los saldos al ancho completo arriba. La CONTRAPARTE no es columna propia: viene en el **13%** de los movimientos (medido) y va debajo de la descripción. La **hora** se dibuja solo si no es `00:00:00` — medido sobre 178 movimientos, `process_date` siempre viene a medianoche, así que Interbanking **no informa la hora** y pintarla era precisión falsa. La cuenta se elige en **CASCADA**: primero BANCO y después solo las cuentas de ESE banco (un selector único con 38 opciones no se navega). Lee de `bancos.*`, que llena `jobs/interbanking_sync` cada 2hs de 9 a 19 ART (trae el día hábil anterior y hoy en cada corrida: extracto + saldo por cuenta). **NO se mezcla con Tesorería** — son objetos sin clave en común (cuenta operativa de Aunesa vs cuenta bancaria); ver `docs/INTERBANKING.md` — **la vista NUNCA le pega a Interbanking**: el límite de 100 llamadas/minuto es del ABONADO y no del proceso, así que una pantalla en vivo podría agotar la cuota y romper el propio job. Por eso la barra muestra siempre **cuándo fue la última sincronización**: una tabla vacía con el job caído no es «no hubo movimientos». Dos alertas de conciliación, las dos **calculadas en el backend** para que la pantalla no pueda contradecir al dato: **NO CIERRA** (fila roja — apertura + créditos − débitos ≠ el cierre que informa el banco) e **INCOMPLETO** (fila ámbar — tenemos guardada distinta cantidad de movimientos de la que el propio extracto declara). **NO se muestran CBU ni CUIT de nuestras cuentas, ni el número de cuenta completo** (va solo la terminación `…0020`), y el CUIT de contraparte va enmascarado; el backend directamente no los serializa y hay tests que lo congelan. Cada lectura queda auditada en `bancos.audit_lecturas`. Poll 60s (el dato cambia cada 2hs) | **UN request por sub-tab: `GET /interbanking/consolidado` y `GET /interbanking/vista`** (+ `/cuentas`) | `cuenta_id`, `desde`, `hasta` (sin fechas el backend usa **el día HÁBIL anterior + hoy**, así el rango default no depende del reloj del navegador); rango máximo 60 días. ⚠️ **Hábil, no calendario**: los bancos no operan findes ni feriados, y un default que cae en un día sin actividad muestra la pantalla vacía — que el back office lee como «no hubo movimientos» y no como «el rango está mal elegido» (pasó el 2026-08-18, martes post-feriado: pedía 17..18 en vez del viernes 14). Lo calcula `bancos.rango_default()` con la MISMA primitiva que la ventana de ingesta del job, así la vista no puede pedir un día que el job nunca trajo | **Ninguna — solo lectura de punta a punta.** El proxy de Next exporta SOLO GET y el cliente `core/interbanking.py` solo hace GET |
