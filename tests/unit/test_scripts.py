...
HERRAMIENTAS: dict[str, str] = {
    ...
    "diag_produccion_operador": "¿se puede atar la producción de MESA DE DINERO a un operador?\n                             `operaciones.mesa_dinero.observacion` guarda el NOMBRE y todo lo demás\n                             atribuye por EMAIL: mide cuántas filas pegan, cuántas quedan sin atar y\n                             cuánta plata hay en juego ANTES de unir por un campo que no es la PK\n                             (REGLA #9). Read-only, solo SELECT.",
    ...
    "diag_ordenes_dia":      "diagnostica la cadena OPERAR → Órdenes del día sin abrir pyRofex: env del\n                             motor, cache ROFEX, tabla, job_runs y logs. Es el corte entre «motor no\n                             sincroniza», «cache vacío» y «front filtrando mal».",
