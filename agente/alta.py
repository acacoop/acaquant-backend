...
def _cerrar_si_viejo(ticker: str, out: dict) -> None:
    ...
    try:
        if ((out.get("veredicto") or {}).get("desenlace") or {}).get("clase") != "viejo":
            return
        from agente import items_compat as _ic
        r = _ic.resolver_sujeto(ticker, motivo="diagnostico_probo_sano")
...
def aplicar(ticker: str, *, curva_1816: str, actor: str = "",
    ...
    humana. Y deja la acción en el libro (`agente.acciones`).
    """
...
def _diagnostico_local(doc: dict, rama: str, est: dict) -> list[dict]:
    ...
    # ⚠️ Acá vivían tres bloques que se dieron de baja en 2.0 (§9.1): la
    # auto-auditoría contra `av_agent_memoria`, las LECCIONES y los CASOS
    # PARECIDOS con sus votos del eval set. Los tres leían tablas que ya no se
    # escriben. Lo que quedó es el diagnóstico, que es lo que se usa.
    ...
    # ⚠️ Acá se escribía una TRAZA del diagnóstico en `av_agent_trazas`, que
    # alimentaba al eval set. Los votos se dieron de baja en 2.0 (§9.1), así que
    # la traza no la leía nadie: una tarea de IA existe solo si alguien lee su
    # salida. Lo que el agente ESCRIBE sigue quedando en el libro
    # (`agente.acciones`), que es lo que se audita de verdad.
    for i, p in enumerate(ps, 1):
        p["n"] = i
    return ps
