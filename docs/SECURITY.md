...
| `DEEPSEEK_API_KEY` | Proveedor LLM **default** (`core/modelos.py`). La usan `agente_emisor`, `asistente_ruteo` y toda la familia mercado del asistente (`asistente_renta_fija`, `asistente_renta_variable`, `asistente_fondos`, `asistente_derivados`, `asistente_financiamiento`, `asistente_dolares`): dato público. Cuenta prepaga, saldo chico: si se filtra, el daño máximo es quemar el saldo. ⚠️⚠️ **Este proveedor SÍ puede entrenar con lo que se le manda**; las tareas con datos del negocio o personales (`asistente_cartera`, `asistente_cliente`, `asistente_operaciones`) salen a OpenAI — ver `config.IA_PERMITE_PROVEEDOR_QUE_ENTRENA`. | platform.deepseek.com → API Keys → regenerar → `.env` → restart API. |
...
recortado a sus últimos dígitos (`asistente/agentes/cliente.py`).
