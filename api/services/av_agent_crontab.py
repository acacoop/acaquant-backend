"""api/services/av_agent_crontab.py — ¿EL CRON DEL REPO ES EL QUE CORRE?
...
def del_repo() -> set[str]:
    ...
    try:
        ...
        logger.warning("av_agent_crontab: no pude leer %s (%s)", p, e)
        return set()
...
def de_la_maquina() -> set[str] | None:
    ...
    try:
        ...
        logger.warning("av_agent_crontab: no pude correr `crontab -l` (%s)", e)
        return None
    ...
    if proc.returncode != 0:
        if "no crontab" in (proc.stderr or "").lower():
            return set()
        logger.warning("av_agent_crontab: `crontab -l` falló: %s",
                       (proc.stderr or "")[:160])
        return None
...
def detectar_crontab() -> list[dict]:
    ...
    try:
        ...
        logger.warning("av_agent_crontab: no pude comparar (%s)", e)
        return []
