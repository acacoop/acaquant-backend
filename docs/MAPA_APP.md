...
| Método | Path | Qué hace | Params | Escribe |
|---|---|---|---|---|
| GET | `/order-book` | Top 5 bid/ask + metrics desde `mercado.market_snapshot`. 3 desenlaces: **200 con puntas**; **200 con `sin_puntas:true`** (fila ≤90s pero libro vacío → devuelve igual el último precio); **202 `subscribing`** si no hay fila o está abandonada (>90s) → registra el ticker en `mercado.adhoc_subscriptions` y el motor lo levanta en ~5s. Valida contra `manager.pyrofex_instruments` (404 si no existe; tabla vacía → subscribe optimista). **429** al cap de 50 suscripciones | `ticker` (**req**), `plazo` (`CI\|24hs\|48hs`, def `24hs`, ignorado si el ticker es full) | Sí por efecto lateral (`adhoc_subscriptions`, TTL 7d rolling, cap 50 con desalojo LRU) — **con método GET** |
...
| GET | `/symbols` | Autocomplete: ILIKE sobre `ticker`/`underlying` en `manager.pyrofex_instruments`. **Excluye FCI** por cficode (`NOT LIKE 'CIO%'`) | `q` (min 2), `limit` (1–50, def 20) | No |
...
| GET | `/mep/{id}/detalle` | Drilldown: doc completo + las 2 patas con su orden live + **timeline de audit** + `precio_compra_al30`, `precio_venta_al30d`, `usd_efectivo`, `ars_operados`, `mep_efectivo`, `slippage_pct` (vs `mep_inicial`), `mep_costo_cliente`, `duracion_ms`. 404 si no existe | path | No |
