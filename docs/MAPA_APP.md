...
| **Análisis Comercial** | KPIs de cupo (transaccional/usado/libre USD al MEP), **Distribución por Nivel 1** (`Ctas. Tot.` · `Activas` · `Enfr.` · `% A+E` · `Op. mes` · AuM, **con fila TOTAL**), desglose por **Nivel 3**, y la tabla **ESTADO COMERCIAL** (cuenta, cliente, estado, días sin operar, AuM, cupo trans., cupo usado, última op, niveles) | `/comercial/analisis`, `/comercial/analisis/detalle` (modal) | Orden por columna (persistido); **3 filtros aditivos por click**: nivel_1 + nivel_3 + estado; los pseudo-estados `SIN_AUM` (aum≤0) y `SIN_OP_YTD` también filtran. Hereda barra madre + Desde/Hasta ("foto al día X"). ⚠️ **La columna ACTIVAS de Distribución pasó a ser el SEMÁFORO** (`estado='ACTIVA'`, días desde la última op): contaba `opero_mtd` —operó en el mes— mientras la tabla de NIVEL 3, en la misma pantalla, contaba el estado. Misma palabra, dos números, ninguno fallaba (REGLA #9). Lo que operó en el mes quedó con su propia columna `Op. mes`, y el `%` es la penetración de **activas + enfriándose** sobre el padrón del segmento = cuánto sigue vivo dentro de la ventana del semáforo | **Export a Excel** (Estado comercial + Distribución nivel 1) |
