...
Este documento explica qué es TradingAV, por qué existe, cómo está construido y cómo se conecta cada pieza. Está escrito para que lo entienda alguien sin perfil técnico.

---

## 1. Qué es y para qué sirve

**TradingAV** es la plataforma cuantitativa interna de la mesa para los mercados argentinos (MERVAL y MAE).

Hace tres cosas en simultáneo, todo el día:
...
**El problema que resuelve**: antes, la mesa armaba planillas Excel manualmente cruzando datos de múltiples fuentes (ROFEX, BCRA, Aunesa, prospectos PDF, Bloomberg, Reuters). Esas planillas se desactualizaban a los 5 minutos y dependían de que alguien estuviera al teclado. TradingAV reemplaza ese flujo con una capa automatizada que vive en el servidor, se actualiza sola y sirve los mismos datos a todos los usuarios al mismo tiempo.
...
- **Carteras de clientes**: posiciones, valuaciones diarias, AuM (assets under management), flujos.

Cada uno de esos dominios genera **decisiones distintas**:
...
TradingAV estructura los datos para que cada una de esas preguntas tenga una respuesta concreta en pantalla y/o un endpoint que la devuelva.
...
Tareas programadas que **no escuchan WebSocket** — corren puntualmente y terminan. Se invocan vía cron.

Ejemplos: snapshot diario de AuM (`jobs.aum`), ingesta de noticias RSS cada 15 min (`jobs.news_ingesta`), descarga de cierres BCRA (`jobs.bcra`), reconstrucción histórica de forwards (`jobs.backfill_forwards`).

La fuente de verdad de qué corre cuándo es **`deploy/crontab.txt`**.
...
### 4.4 Frontend (`acaquant-web`, repo separado)
...
```
  ...
  └────┬─────┘  └────┬─────┘    └────┬─────┘    └────┬─────┘   └────┬─────┘
       ...
       └─────────────┴───────┬───────┴───────────────┴──────────────┘
                ...
                ▼                            ▼
   ┌─────────────────────┐         ┌────────────────────┐
   │ FastAPI (api/)      │         │ Cron jobs (jobs/)  │
   │ ~70 endpoints       │         │ AuM, BCRA, News,   │
   │ @cached(ttl=60–300) │         │ Carteras, Cleanup  │
   └──────────┬──────────┘         └──────────┬─────────┘
              │ HTTP                          │ escribe
              │                               ▼
              │                    ┌──────────────────────┐
   ...
   │  (encripta + valida JWT con OTP por email)     │
   └────────────────────┬───────────────────────────┘
                        │
                        ▼
              ┌──────────────────────┐
...
**Lectura rápida**:
1. ROFEX manda trades → un único `WebSocketManager` los reparte a 8 motores que corren en paralelo.
2. Cada motor procesa lo suyo y escribe en MongoDB.
3. En paralelo, jobs cron escriben otras colecciones (carteras, news, macro).
4. La API lee Mongo (siempre con `read-only`) y expone endpoints.
