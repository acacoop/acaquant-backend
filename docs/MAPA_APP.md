...
| **Interbanking** | Los bancos de ACA, para CONCILIAR. **DOS sub-tabs** y **UN día** (el selector es una sola fecha, compartida por las dos; sin fecha el backend usa HOY en hora argentina, así el default no depende del reloj del navegador). **CONSOLIDADO BANCOS** (default): UNA fila por cuenta agrupadas bajo el **nombre del banco como título**, con **SALDO AL INICIO** (apertura de ese día) · **SALDO AL CIERRE** · **VARIACIÓN** · **GASTOS BANCARIOS** · **MOVS.** El agrupado por banco existe solo para navegar 38 cuentas: **NO hay subtotales por banco ni totales por moneda** (los sacó el back office 2026-08-18 — no los usaba, y sin ellos la vista arranca directo en la tabla). **El CIERRE tiene DOS fuentes y la celda rotula cuál**: sin rótulo = EXTRACTO; **«saldo»** = lo informa el banco pero la cuenta no se movió ese día y no hay extracto que lo respalde (`bancos.saldos` — sin esto iba «—», porque el extracto solo devuelve los días CON actividad); **«≠»** = el banco informa las dos y no coinciden, con la diferencia en el tooltip (hallazgo de conciliación, no un número a elegir). Sin ninguna de las dos va «—», nunca 0, y la barra dice cuántas están así. **GASTOS BANCARIOS viene `null` en todas las filas**: la regla de qué movimiento es un gasto NO está definida (`GASTOS_BANCARIOS_CODIGOS` vacío) — se muestra «—» y jamás cero, porque «no sabemos» y «no hubo gastos» son cosas distintas. **DETALLE POR CUENTA**: una cuenta repartida **1/3 · 2/3** — izquierda **EXTRACTO** (apertura · créditos · débitos · cierre), derecha **MOVIMIENTOS** con 8 columnas (FECHA · DESCRIPCIÓN · CONCEPTO · COD OP · COD OP BCO · COMPROBANTE · SUCURSAL · IMPORTE). La CONTRAPARTE no es columna propia: viene en el **13%** de los movimientos (medido) y va debajo de la descripción. La **hora** se dibuja solo si no es `00:00:00` — medido sobre 178 movimientos, `process_date` siempre viene a medianoche, así que Interbanking **no informa la hora**. La cuenta se elige en **CASCADA** (banco → cuenta de ESE banco). Lee de `bancos.*`, que llena `jobs/interbanking_sync` cada 2hs de 9 a 19 ART (día hábil anterior + hoy: extracto + saldo por cuenta) — **la vista NUNCA le pega a Interbanking** (el límite de 100 llamadas/minuto es del ABONADO), por eso la barra muestra siempre cuándo fue la última sincronización. **NO se mezcla con Tesorería** — objetos sin clave en común; ver `docs/INTERBANKING.md`. **NO se muestran CBU ni CUIT de nuestras cuentas, ni el número completo** (va la terminación `…0020`), y el CUIT de contraparte va enmascarado; hay tests que lo congelan. Cada lectura queda auditada en `bancos.audit_lecturas`. Poll 60s. **Clic en una celda con dato = se copia**. El **número de cuenta va ENTERO** (2026-08-18); el **CBU no sale nunca**. Barra: solo «Última actualización DD/MM/AAAA HH:MM». `bancos.*` retiene **3 fechas** | **UN request por sub-tab: `GET /interbanking/consolidado` y `GET /interbanking/vista`** (+ `/cuentas`) | `cuenta_id`, **`fecha`** (una sola; default = **día HÁBIL ANTERIOR a hoy**, el que está cerrado; futura → 400) | **Ninguna — solo lectura de punta a punta.** El proxy de Next exporta SOLO GET y `core/interbanking.py` solo hace GET |
