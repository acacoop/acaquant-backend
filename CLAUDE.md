...
- `manager` — `manager_users`, `role_matrix`, `role_audit`, `grupos`, `job_runs`, `health_reports`, `watchdog_alertas`, `pyrofex_instruments`/`pyrofex_discovery`; `uso_modulos` (telemetría de uso usuario × módulo × hora — contador agregado que flushea el middleware `api/telemetria.py`, lo lee `GET /api/manager/uso`); `asistente_chats`/`asistente_mappings` (ASISTENTE DE NEGOCIO — QuantAI P7: transcript real + mapping ficha↔identidad de la aduana `core/pii_gateway.py`; módulo RBAC `asistente`, admin-only; NO tiene endpoint propio — es la vista `negocio` del copiloto, cerebro en `api/services/asistente.py`); `pedidos` (BUZÓN: sugerencias/faltantes/bugs que la mesa le dice al copiloto — tool `registrar_pedido`; los tría con IA `jobs/pedidos_triage.py` y se aprueban con botones de Telegram vía `jobs/pedidos_inbox.py`; export a `docs/PEDIDOS.md` con `python -m scripts.gen_pedidos`, comando `/pedidos`. Circuito y postura de seguridad: **`docs/PEDIDOS_FLUJO.md`**) y `telegram_cursor` (offset de lectura del bot).
...
| Buzón de pedidos (triaje IA + aprobación Telegram) | `PEDIDOS_FLUJO.md` · `PEDIDOS.md` (auto-gen) |
