...
| 8 | **RENTA VARIABLE** | `/renta-variable` | `renta-variable` | admin, trader, sales, asistente_comercial, invitado | Panel CEDEARS: tabla de CEDEARs en ARS live (INTRA, 1D, USD real, $ operado) + CCL. La mitad derecha está vacía (refactor en curso 2026-09-10). |
...
- **Refactor en curso (2026-09-10, paso 1):** la vista es un panel **CEDEARS** (izquierda, 50 %) con la tabla en ARS, y la **derecha queda VACÍA a propósito** hasta el próximo paso. Se fueron el switch CEDEAR/ADR, las columnas RUBRO / SPREAD / VWAP, el panel MÉTRICAS entero (PULSO · PIVOTS/VOL · RETORNOS) y el panel CHART & RETORNOS. Los endpoints por ticker (`/pivot`, `/quant`, `/returns`) siguen vivos en el backend **sin consumidor** hasta que se decida el lado derecho; los componentes `metricas-panel.tsx`, `pivot-points-panel.tsx`, `retornos-chart.tsx` y `ticker-chart-panel.tsx` quedaron en el repo sin importador por la misma razón.

| Tab | Qué muestra | Endpoints | Filtros | Escrituras |
|---|---|---|---|---|
| **CEDEARS** (panel izq., único) | Tabla en ARS live: TICKER, NOMBRE, LAST, INTRA %, 1D %, **USD %** (retorno real descontando CCL) y **$ OPERADO** (`total_money` = plata operada en el día, el MISMO dato y formato que ranquea VOLUMENES en `/trading`; reemplazó al VOL nominal). KPI CCL inline. Click en fila = ticker elegido (hoy no alimenta nada; es el enganche del lado derecho futuro) | `GET /api/scanner/cedears` (poll 2s), `GET /api/scanner/ccl` (5s) | **buscador** de texto (ticker o nombre, client-side); **orden** por click en CUALQUIER header (default INTRA desc; nulls siempre al final) | Ninguna |
| (Derecha) | **Vacío.** Panel reservado | — | — | — |
