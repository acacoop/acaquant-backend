...
- [[api.ext]] — api/ext — API EXTERNA para accionistas (`/ext/v1`). Ver docs/API_EXTERNA.md.
- [[api.ext.app]] — api/ext/app.py — la sub-app `/ext`. Superficie COMPLETA de la API externa.
- [[api.ext.auth]] — api/ext/auth.py — autenticación y SCOPE de la API externa.
- [[api.ext.db]] — api/ext/db.py — las cuatro tablas del schema `ext` (quién entra y qué ve).
- [[api.ext.lectura]] — api/ext/lectura.py — LA ÚNICA puerta por la que la API externa lee datos.
- [[api.ext.ratelimit]] — api/ext/ratelimit.py — cuota de la API externa, por CLIENTE.
...
- [[api.routers.agente]] — `api/routers/agente.py` — el AV AGENT por HTTP. Doc: `docs/AGENT_2.0.md`.
- [[api.routers.analitica]] — Router Analítica — Tier 1 + Tier 2 tools del asistente expuestas como HTTP.
- [[api.routers.ap5]] — Router AP5 — /api/ap5 (vista POSICIONES Y DIFERENCIAS).
...
- [[api.routers.ia]] — api/routers/ia.py — el módulo IA: hoy, SOLO el briefing de apertura.
...
- [[api.routers.market]] — Router Market: watchlist quotes + titulares Eikon.
...
- [[api.services.ap5_posiciones]] — api/services/ap5_posiciones.py — POSICIONES Y DIFERENCIAS.
...
- [[api.services.bono_detalle]] — api/services/bono_detalle.py — LA FICHA DE UN BONO, en un request.
...
- [[api.services.conoce_cliente_sql]] — api/services/conoce_cliente_sql.py — tab CONOCÉ A TU CLIENTE (vista OPERADORES).
...
- [[api.services.cuantitativo_sql]] — api/services/cuantitativo_sql.py — tab ANÁLISIS CUANTITATIVO (vista OPERADORES).
...
- [[api.services.perfil_cliente_sql]] — api/services/perfil_cliente_sql.py — la ficha operativa de UN cliente.
...
- [[api.services.profundidad_sql]] — api/services/profundidad_sql.py — tab PROFUNDIDAD DE CLIENTES (vista OPERADORES).
...
- [[api.services.rv_motor]] — api/services/rv_motor.py — matriz de correlación del universo USD (Renta Variable).
...
- [[api.services.simular_inversion]] — Simular Inversión — ¿qué cobro si pongo $X en este bono a este precio?
