...
<!-- AUTOGEN:resumen -->
- **534 endpoints** montados en `api.main.app`, en **35 routers**.
- **198 escriben** (POST/PUT/PATCH/DELETE); 336 son de solo lectura.
...
| `/api/scanner` | 5 | 0 | `renta-variable` · 2 rutas con gate extra | — |  |
| `/api/titulos` | 2 | 0 | — | `portfolios` | ⚠️ |
| `/api/trading` | 6 | 0 | `trading` | `trading` |  |
...
| 14 | **OPERACIONES** | `/operaciones` | `operaciones` | admin, trader, asistente_comercial | Volumen y arancel de boletos de mercado, con búsqueda de cuenta por `id_cuenta` estable (la denominación histórica queda solo como etiqueta/cross-filter), más las verticales AGRO / DÓLAR FUTURO / POSICIONES Y DIFERENCIAS y los depósitos/extracciones. |
...
| 17 | **BACK OFFICE** | `/back-office` | `back-office` | admin, trader, sales, asistente_comercial, back_office | Operación diaria del back office: SENEBIS, tenencia valorizada, títulos en alquiler, Tesorería (caja del día), títulos a enviar/recibir al mercado y acreencias de clientes, extractos bancarios de Interbanking, saldos de cuentas comitentes y COMISIONES FCI (lo que cobra ACA Valores por la tenencia de fondos). |
...
| `/back-office` | Senebis · **Tenencia Valorizada** · Títulos en Alquiler · Contabilidad · Tesorería · Interbanking · Títulos / Mercado · Acreencias Clientes · Saldos de Cuentas Comitentes · **Custodia** (TENENCIAS · MOVIMIENTOS) | `backoffice.tab` | ⚠ El orden VISUAL pone Senebis primero pero **el default es la 2ª tab**. Senebis: **Órdenes** · Excel Quantex · Excel MAE. Tesorería: **movimientos** · bancos · cheques · mercados · banco a banco. Alquiler: **Portfolio Alquiler** · Marcas por cuenta |
...
- **Archivos front**: `renta-fija-live.tsx` (shell 2×2 + poll unificado), `curvas-tab.tsx` (tab CURVAS: filtro de TIPO de emisor —**excluyente**, un solo tipo activo desde 2026-09-10— + filtro de EMISOR por nombre + filtro de TEA + las dos columnas), `bonos-table.tsx`, `bono-modal.tsx` (**la FICHA del bono** — tasas arriba a todo el ancho; abajo, a la izquierda la ficha vertical con el cronograma debajo, a la derecha el gráfico del flujo a toda la altura), `renta-fija-table.tsx`, `libro-panel.tsx`, `curvas-chart.tsx`, `fair-value-view.tsx`, `fair-value-modal.tsx`, `forwards-panel.tsx`, `forward-matrix.tsx`, `forward-matrix-zscore.tsx`, `breakevens-block.tsx`.
...
| **RENTA FIJA → LIBRO** (**ventana flotante**, botón a la derecha del header ARS) | Time & sales intradía de UN bono; solo trades de HOY (sin fallback a sesión vieja). **No es una tab ni una pill**: abre una ventana que se arrastra y se redimensiona, por `createPortal` al body → la tabla y la curva ARS siguen enteras y visibles detrás. No es modal (sin backdrop, el fondo sigue clickeable); cierra con ✕ o **Esc**. Geometría persistida en `localStorage` (`rentaFija.libro.ventana`); abierto/cerrado NO se persiste | `GET /api/trades?instrumento=` → `/api/cotizaciones/historico/trades`, poll 5s con dedupe | Buscador/dropdown de instrumento. Universo = **el lado ARS entero** (no una pill: el tape se busca por ticker), deduplicado por instrumento y limitado a los que tienen `last_price`. Respeta los filtros de arriba (TIPO · EMISOR · TEA) → con el default (TIPO=SOBERANO, sin emisor tildado) son los soberanos ARS | ninguna |
...
| **CURVAS → filtro de EMISOR** (botón `EMISOR`, entre la fila de TIPO y el `TEA ≥`) | Emisor por NOMBRE (YPF, Pampa, Telecom…), multi-select con buscador y el conteo de **bonos** de cada uno. Nace para **CORPORATIVO**, donde la tabla son ~134 ONs de decenas de emisores y el piso de tasa solo no alcanza. El catálogo son los emisores de **lo que el TIPO ya dejó pasar** (no el universo), ordenados por cantidad. Aplica a la misma cadena que el de TEA: tabla, gráfico, contadores de las pills y universo del LIBRO | ninguno: **client-side** sobre `curvas-vista`, que ya trae `emisor` (el nombre) y **`emisor_key`** (la clave con la que se agrupa) | **Vacío = TODOS** (acá no es ambiguo: el botón dice el emisor activo, o cuántos hay). Los bonos **sin emisor cargado** son su propio grupo `(SIN EMISOR)` — no se esconden. ⚠ La clave la manda el BACKEND (`upper(btrim(emisor))`, la misma con la que la base resuelve la industria): agrupar por el string suelto partiría `'YPF '` de `'YPF'` en dos chips que cuentan bien por separado (REGLA #9). Al cambiar el TIPO se **sueltan** los emisores tildados que ese TIPO ya no muestra. NO se persiste | ninguna |
| **CURVAS → filtro de TEA** (botón `TEA ≥`, el último de la fila) | Piso de tasa: deja solo los bonos que rinden de ahí para arriba. Presets 5/7/10/15/25/35/45/60 % + input libre. Aplica a **las dos columnas, la tabla, el gráfico, los contadores de las pills y el universo del LIBRO** — una sola fuente, así la pantalla no puede contradecir a sus propios controles | ninguno: filtra **client-side** sobre los bonos que ya trae `curvas-vista` (el emisor también) | ⚠ Es **GLOBAL y las dos escalas NO son comparables** (ARS 30-60 %, USD 5-15 %): un mismo piso no filtra nada a la izquierda y sí a la derecha. Decisión tomada a conciencia — por eso el botón MUESTRA el número activo. Los bonos **sin TEA o con `tasa_ruido`** quedan afuera y se cuentan aparte (`−N` en el botón): "no llega al piso" y "no se puede comparar" no se muestran igual. NO se persiste | ninguna |
| **CURVAS → FICHA DEL BONO** (**modal**, click en cualquier fila de la tabla) | Izquierda la **ficha** (emisor, industria, moneda, ley, emisión/vto, VN, CER de emisión / pago final cuando aplican — sin símbolo, tipo, tipo emisor, ajuste ni cupón desde 2026-09-10); derecha, a toda la altura, la **tabla del cronograma** (fecha, amortización, interés, total, **ACUMULADO** = lo cobrado hasta ese pago sumando solo los futuros, residual; por 100 VN, los pagos vencidos apagados) + pie con pagos futuros / próximo / último / total a cobrar + **bloque de tasas y riesgo por PATA** arriba (TEA/TNA/TEM, duration, mod dur, convexity, paridad, margen s/TAMAR, TC BE, con su procedencia LIVE vs 1816). **Sin gráfico del flujo desde 2026-09-10** (paso 27 de `RENTA_FIJA.md`: ni dos escalas ni una se leían). Toggle SOLO FUTUROS (def. ON). Cierra con ✕, Esc o click afuera | `GET /api/cotizaciones/bono/{ticker}` (TTL 30s) → `api/services/bono_detalle.py` | El toggle SOLO FUTUROS. Un **dual** muestra sus DOS patas, cada una con su tasa: mostrar una sola es el bug que la tabla ya resolvió | ninguna |
...
- **Archivos front**: `scanner-view.tsx` → `cedears-scanner-table.tsx`. **Proxy Next `/api/scanner/[...path]` es solo GET** → cero escrituras posibles desde la vista.
- **Refactor 2026-09-10 (pasos 1 y 2):** la vista son DOS paneles 50/50: **CEDEARS** (izquierda, la tabla en ARS) y **ADR** (derecha, TradingView del **subyacente en USD** del papel elegido — `underlying` de la fila, YPFD→YPF; es el ADR y no el CEDEAR a propósito). Sin click, el chart arranca con el primer papel del orden por defecto (INTRA desc). Se fueron el switch CEDEAR/ADR, las columnas RUBRO / SPREAD / VWAP, el panel MÉTRICAS entero (PULSO · PIVOTS/VOL · RETORNOS) y el panel CHART & RETORNOS (`metricas-panel.tsx` y `ticker-chart-panel.tsx` borrados). **Paso 3 (mismo día):** `/returns` y `/quant` se **borraron** del backend (con `retornos-chart.tsx`, la tab VOL & BETA y el test) — nadie los consumía y Research ya tiene los retornos posta (Reuters). `/pivot` sobrevive porque lo consume la ventana **PIVOTS** de TRADING → MONITOR (`pivot-points-panel.tsx`, ver esa tab). Y `/cedears` dejó de mandar `adr_*`, `rubro` y `es_ia`: ningún consumidor los leía (front, `monitor_sql`, `trading_pivots`) y con ellos se fue una lectura de `precios_acciones` para todo el universo + otra de `adr_snapshot` en cada hit de un poll de 2 s.

| Tab | Qué muestra | Endpoints | Filtros | Escrituras |
|---|---|---|---|---|
| **CEDEARS** (panel izq., único) | Tabla en ARS live: TICKER, NOMBRE, LAST, INTRA %, 1D %, **USD %** (retorno real descontando CCL) y **$ OPERADO** (`total_money` = plata operada en el día, el MISMO dato y formato que ranquea VOLUMENES en `/trading`; reemplazó al VOL nominal). KPI CCL inline. Click en fila = ticker elegido → cambia el chart ADR de la derecha | `GET /api/scanner/cedears` (poll 2s), `GET /api/scanner/ccl` (5s) | **buscador** de texto (ticker o nombre, client-side); **orden** por click en CUALQUIER header (default INTRA desc; nulls siempre al final) | Ninguna |
| **ADR** (panel der.) | TradingView Advanced Chart del subyacente USD (`underlying` de la fila; fallback `ticker_corto`), velas diarias, sigue el tema. Componente genérico `tradingview-chart.tsx` (el de HOME). Título `ADR · NVDA`, sub `CEDEAR NVDA · Nvidia`. Datos de NYSE/NASDAQ con el delay del widget gratuito | ninguno del backend | el widget permite cambiar símbolo e intervalo adentro | Ninguna |

#### Endpoints — `scanner.py` (`/api/scanner`, gate `renta-variable`; además en `GUEST_PATH_PREFIXES`)
| Método | Path | Qué hace | Params | Escribe |
|---|---|---|---|---|
| GET | `/cedears` | Master + snapshot live joineado: `ticker_corto, nombre, underlying, ratio_cedear, last, open, high, low, close, bid, offer, spread, spread_pct, vwap, volume, total_money, intraday_pct, vs_1d_pct, vs_1d_usd_pct, updated_at` (`@cached ttl=2`). **Sin `adr_*` / `rubro` / `es_ia` desde 2026-09-10** | — | No |
| GET | `/ccl` | CCL live + var 1D. Cae al último cierre si el snapshot tiene >60s (`ttl=5`) | — | No |
| GET | `/pivot/{ticker}` | Pivots del **ADR en USD** en 4 timeframes (`precios_acciones`); el `last` se pisa con el live del ADR (`last_source: live\|eod`) (`ttl=60`). Consumidor: la ventana **PIVOTS** de TRADING → MONITOR. NO es `/api/trading/pivots` (CEDEAR en ARS) | path (resuelve `ticker_corto → underlying`, ej. YPFD→YPF) | No |
...
`mercado.precios_acciones` (`jobs/precios_acciones_daily.py` 22 UTC — base de `/pivot`)
...
#### Notas / rarezas
- **2 endpoints del scanner no tienen consumidor en el frontend**: los admin-only `/day-trading` y `/companeros` — la vista `/trade-lab` que los alimentaba **no existe** en el checkout (solo quedan comentarios que la mencionan). Backend vivo, UI muerta.
...
- **Archivos front**: `trading-shell.tsx` → `trading-view.tsx`, **`monitor-view.tsx`**, `intraday-view.tsx`; sub-componentes `trading-radar-panel.tsx` (→ movers / volumen / pivot-radar), `live-intraday-chart.tsx`, `cedears-scanner-table.tsx` (reusada en modo `compact` + `soloCedear`). Proxy: `/api/trading/[...path]` (**GET-only** desde el 2026-09-04: el único POST era el del cuaderno de PnL).
...
| **MONITOR** (tab nueva 2026-09-04) | Volumen operado **por precio**. Filtro madre **RENTA FIJA / RENTA VARIABLE** (nunca se mezclan: cambian universo, unidades y ventanas). Izquierda 20 % el universo de la clase; derecha 50/50: precio del tape arriba (con VWAP y último) y abajo el histograma de nominales por precio, con **POC** (más operado), **VAL** (piso del 70 %) y **VAH** (techo del 70 %), cada uno como KPI propio y nombrado — en el gráfico son siglas sueltas. El volumen **no se parte por lado** | `GET /api/trading/monitor/universo?clase=`, `GET /api/trading/monitor?clase&ticker&ventana&buckets` | **Clase** (rv def) · **curva** en renta fija (TASA FIJA/CER/TAMAR/…, def TODAS; el catálogo y el conteo los manda el backend, y un dual aparece en las DOS) · **ventana** (rv: HOY/5R/20R · rf: HOY/3R/5R; **abre en la más larga — 20R en rv, 5R en rf**, y el default lo declara el BACKEND en `POR_DEFECTO`, no el front) · buscador del rail · buckets 8-60 (def 26) · **botón PIVOTS** (solo renta variable, 2026-09-10): abre una `VentanaFlotante` con `pivot-points-panel.tsx` = máximo / mínimo / cierre del período previo + R3…S3 con distancia al last, en DIARIO / SEMANAL / MENSUAL / ANUAL, del **ADR en USD** (`GET /api/scanner/pivot/{ticker}`, gate `renta-variable`, poll 60 s). Es el panel que tenía `/renta-variable`; NO toca los pivots en ARS de la tab PIVOTS | Ninguna |
...
suma `cash`) · `GET /universo` ·
`GET /monitor/universo?clase=rv|rf` (devuelve `{clase, ventanas[], curvas[], items[]}` — **las ventanas y las
curvas las manda el backend**, así el selector no puede ofrecer una que el service rechace ni clasificar un bono
distinto que la tab CURVAS: la taxonomía sale de `curvas_vista.pills_del_master()`, el mismo lugar. Un bono sin
pill acordada —badlar/tpm/caución— cae en **OTROS** y NUNCA se oculta) ·
`GET /monitor?clase&ticker&ventana&buckets` (`ventana` vacía → la que la clase declara por defecto) (`{fuente, aproximado, paso_min, serie[], buckets[], poc, val,
...
cuaderno) · **`mercado.cedears_bars_1m`** (MONITOR multi-rueda, ver abajo).
**La vista no escribe NINGUNA tabla** desde que se borró PNL HISTÓRICO (2026-09-04).
...
| **ARANCELES** | Serie de aranceles por periodo + tabla izq por dimensión configurable + tabla der por cliente + por instrumento | `/ops/aranceles`, `/ops/fechas`, `/ops/meta`, `/ops/segmentos`, `/comercial/operadores` | Mismo modo de rango; moneda ARS/USD; **selector de dimensión** `dim` = `nivel3`\|`operacion`\|`mercado`\|`operador`; segmento y operador single-select; cross-filter por `sel_dim`/`cuenta`/`instrumento`; botón **ALL** → `serie_full=true` (por default la serie se acota a ~18 m / 550 días) | Ninguna. **Export a Excel** (botón `⬇ XLS` en la barra → `GET /ops/aranceles/export`, .xlsx generado en el backend con openpyxl): 3 hojas — **Consolidado** (la tabla izquierda en sus CUATRO dimensiones, un bloque por cada una con fila TOTAL), **Por cliente** y **Por instrumento** — cada una con cabecera Desde/Hasta como celdas de FECHA, moneda y filtros activos; importes y conteos como NÚMERO, claves como TEXTO. ⚠️ Difiere de la pantalla a propósito: **todos los filtros aplican a las tres hojas** (en pantalla cada tabla ignora su propia selección para poder des-seleccionar), así los bloques cierran contra el mismo total |
...
| GET | `/ops/aranceles/export` | `.xlsx` (bytes) de ARANCELES: hojas Consolidado (4 dimensiones) · Por cliente · Por instrumento, con los MISMOS filtros que `/ops/aranceles` aplicados a todo. Sin cache; 501 si falta openpyxl | `moneda`, `desde`/`hasta`(**req**), `cuenta`, `instrumento`, `sel_dim`, `segmento`, `operador`, `dim` + `scope` | No |
...
| POST | `/financiamiento/calculadora/lote` | La planilla para un LOTE de cheques/pagarés con el mismo instrumento y aval: fila por fila (mismas fórmulas que el simple) + totales + plazo promedio ponderado por nominal + **CFT del lote** (sobre el neto final total y el plazo ponderado; se calcula también SIN aval, porque el reporte se manda al cliente en operaciones directas) + flujos agrupados por vencimiento. Tope 50 filas. **NO PERSISTE NADA**: el lote vive en el navegador del comercial (localStorage) y solo por el día. Números clavados en `tests/unit/test_financiamiento_calc.py` | — | No (calcula, no escribe) |
...
| **Análisis Comercial** | KPIs de cupo (transaccional/usado/libre USD al MEP), **Distribución por Nivel 1** (`Ctas. Tot.` · `Activas` · `Enfr.` · `% A+E` · AuM, **con fila TOTAL**), desglose por **Nivel 3**, y la tabla **ESTADO COMERCIAL** (cuenta, cliente, estado, días sin operar, AuM, cupo trans., cupo usado, última op, niveles) | `/comercial/analisis`, `/comercial/analisis/detalle` (modal) | Orden por columna (persistido); **3 filtros aditivos por click**: nivel_1 + nivel_3 + estado; los pseudo-estados `SIN_AUM` (aum≤0) y `SIN_OP_YTD` también filtran. Hereda barra madre + Desde/Hasta ("foto al día X"). ⚠️ **La columna ACTIVAS de Distribución pasó a ser el SEMÁFORO** (`estado='ACTIVA'`, días desde la última op): contaba `opero_mtd` —operó en el mes— mientras la tabla de NIVEL 3, en la misma pantalla, contaba el estado. Misma palabra, dos números, ninguno fallaba (REGLA #9), y sumarle ENFRIÁNDOSE —que siempre fue estado— habría totalizado dos cosas distintas. El `%` es la penetración de **activas + enfriándose** sobre el padrón del segmento = cuánto sigue vivo dentro de la ventana del semáforo | **Export a Excel** (Estado comercial + Distribución nivel 1) |
| **Profundidad de Clientes** | **TRES solapas** (conmutador fino): **Por mes** (default) · **Alta de Cuentas** · **Análisis Cuantitativo**.<br>**POR MES** — UNA tabla a ancho completo, **una fila por período** — **MES** (`jul-25`) o **TRIMESTRE** (`Q3-25`), conmutable (`granularidad`, persistida; en trimestral los extremos se **anclan** al trimestre completo: media fila se lee como una caída del negocio) — desde `PROFUNDIDAD_INICIO` hasta el período en curso: CLIENTES · **ALTAS** · CON AuM · SIN AuM · ACTIVOS · RATIO ACTIV. · ARANCELES · ARANC./ACTIVO · AuM. **ALTAS** = cuentas dadas de alta DENTRO del período (el FLUJO, por `fecha_alta_legajo`) contra CLIENTES que es el STOCK acumulado a su último día; salen de la MISMA lista de fechas —una query— así que el stock de un período es el del anterior más sus altas. Fila **TOTAL** al pie que suma **solo lo sumable** (altas y aranceles): clientes y AuM son stocks y ACTIVOS doble-contaría a quien operó en más de un período → esas celdas van en `—` con el motivo en el tooltip. ⚠️ El total de altas **no da** el total de clientes: las altas anteriores a la primera fila ya estaban en la base, y va dicho en `meta.advertencias`. El modal recibe la MISMA `granularidad` que la tabla (si no, abriría un mes de un trimestre y el total no cerraría). **Todo medido al ÚLTIMO día del mes**; los flujos, sobre el mes completo. La plata va **entera y sin decimales** (`$1.234.567`), no con el compacto del resto de Comercial <br>**ALTA DE CUENTAS** — el histórico **COMPLETO** de altas de `clientes.comitentes`: el **gráfico ocupa el 100 % del alto** y la tabla con los mismos números (período · altas · acumulado · % del total, con fila TOTAL) vive detrás de un pill **GRÁFICO ↔ TABLA**. Granularidad **mes / trimestre / año**. El gráfico es **combinado**: barras = altas del período (eje izquierdo) + **línea roja = acumulado (eje Y DERECHO)**, con el número escrito en cada punto. ⚠️ **Es un gráfico de DOBLE EJE, elegido por el user tras plantearle el problema**: con dos escalas, el punto donde la curva «cruza» a las barras lo decide la escala que se eligió y no los datos. Mitigado con lo único que lo mitiga: **cada eje lleva el color de SU serie** (izq. `--t-brand`, der. `--t-neg`), el acumulado lleva **su número en cada punto** (así no depende de medirlo contra el eje) y la leyenda está siempre. Con granularidad MES y muchos años las etiquetas se pisan → ahí va TRIM. o AÑO. Leyenda **abajo**. Botón **⧉ COPIAR IMAGEN** (solo en la vista gráfico): arma un PNG con la misma barra azul + logo del reporte fin de día de Interbanking. ⚠️ **Serializa el `<svg>` que recharts YA dibujó** (`svg.recharts-surface`) en vez de redibujar el gráfico —dos dibujos del mismo gráfico se separan con el tiempo—, pero un SVG fuera del documento **no hereda CSS**: las `var(--t-*)` no resuelven y el navegador pinta NEGRO sin avisar, así que se reemplazan por su valor del tema **CLARO** (leído de un `div.light` invisible; un mail con fondo negro se imprime pésimo). Y la **leyenda de recharts es HTML, no SVG**: no viaja en la serialización y se dibuja en el canvas desde lo que declara quien llama. Ejes y etiquetas en **negrita** (`fontWeight` va en el tick de recharts, que lo escribe como atributo del `<text>` → viaja en el SVG; una regla CSS de la app no llegaría a la imagen). Módulo: `src/lib/grafico-imagen.ts` (hermano de `reporte-imagen.ts`, mismo fallback copiar→descargar). El pie de la imagen se parte en líneas midiendo contra el ancho real y el alto del lienzo las cuenta ANTES de dibujar. La única advertencia que queda es la del caso riesgoso (`solo_activas=true`, el pasado se achica); contar todas las comitentes es el default y no se anuncia. **Cuenta TODAS las comitentes por default, no solo las `Activa`** (toggle SOLO ACTIVAS): un alta de 2019 sigue siendo un alta de 2019, y filtrar por estado haría que el pasado se achique cada vez que se cierra una cuenta. El rango arranca en la PRIMERA alta que existe (no en `PROFUNDIDAD_INICIO`) y termina **en HOY, no en la última alta**: los períodos sin altas son el dato. Es una solapa aparte y no una columna de POR MES —que también tiene ALTAS— porque los rangos son distintos a propósito: aquélla es el ejercicio en curso, ésta es toda la historia | `/comercial/profundidad`, `/comercial/profundidad/detalle` (modal), `/comercial/altas-historico` | **NO usa el Desde/Hasta** (su eje ES el tiempo → el control se esconde). Sí hereda la barra madre completa, y los filtros activos se dibujan como **chips arriba de la tabla**. **MultiSelect OPERACIÓN propio de la vista** (no de la barra madre): acota SOLO activos/ratio/aranceles/aranc.-por-activo; clientes, con AuM, sin AuM y AuM siguen siendo la base entera. Moneda ARS/USD — en USD, **al MEP del mes**, no al de hoy | **Export a Excel** (la tabla; y otro dentro del modal) |
...
| **Informe** | 4 cuadrantes de toda la mesa: **Q1** cuentas por segmento (CINCO modos: `cuentas` / `ctas. operativas mes` / `op. año` / `activas + enfr.` / `aranceles`; el % es **penetración sobre la base de ESE segmento**), **Q2** ranking volumen+aranceles por comercial, **Q3** aranceles por segmento, **Q4** detalle del segmento (clientes) | `/comercial/informe`, `/informe-segmento`, `/informe-aranceles-segmento`, `/informe-segmento-detalle`, `/informe-cliente-ops` (modal; param `ventana=mes|ano|reciente`) | Desde/Hasta, moneda, filtros madre (**el `operador` madre va SOLO al ranking Q2**; en Q1/Q3/Q4 `operador` es el drill-down del comercial clickeado). Click en comercial re-scopea Q1/Q3/Q4; click en segmento filtra Q4; **click en un cliente de Q4 → modal con SUS boletos del MES del corte** (`/informe-cliente-ops`: ventana y predicado idénticos a CTAS OPS y al filtro, así una fila que el filtro dejó pasar nunca se abre vacía). ⚠️ La tab OPERACIONES de Q4 **se dio de baja** (2026-08-31) y con ella la lista de boletos que viajaba en `/informe-segmento-detalle` — eran hasta 20.000 filas sin dueño, capeadas, que no contestaban ninguna pregunta; era el viaje más caro del endpoint. La tabla de clientes incluye ahora las cuentas que **operaron sin dejar arancel** (pill «operó», arancel en cero): sin ellas el contador del filtro daba MENOS que CTAS OPS —medido en pantalla, 30 contra 33— porque la lista nacía de un `arancel > 0` y el contador de arriba no; **filtro de Q4 con CUATRO recortes** (`Todas` / `Operativas <mes>` / `Operaron <año>` / `Activas + enfr.`) — la ventana va escrita en el botón porque «operó» sin decir cuándo es la ambigüedad que hacía que dos pantallas dieran distinto: una cuenta que operó en marzo y paró **no** operó este mes y **sí** operó este año (en ANÁLISIS COMERCIAL figura `DORMIDA`, que mide días desde la última op — otra pregunta). ⚠️ **El Desde NO mueve ninguna de las dos ventanas**, ni CTAS OPS: solo el volumen y el arancel. Por eso el año es una pill propia y no «poner Desde=1/1». **Click en una barra de Q1** en modo op. mes / op. año baja ese segmento a Q4 con el MISMO filtro puesto (ver quiénes, no solo cuántos); antiguo toggle en Q4·Clientes (deja las cuentas con ≥1 boleto en el mes del corte — el MISMO predicado que CTAS OPS; el flag `opero_mes` viaja por fila, así filtra sin refetch y no puede quedar desfasado de lo dibujado). ⚠️ **A propósito NO es un filtro de la vista**: si achicara también el universo, el % de Q1 (`operativas / cuentas del segmento`) daría **100 % en todos los segmentos** y dejaría de significar nada — misma regla que el filtro de operación de Profundidad, que toca el numerador y nunca la base; botón `?` de ayuda. ⚠️ **EL SEMÁFORO ENTRÓ AL INFORME COMO UNA SOLA LECTURA (`activas + enfr.`)** — ACTIVA y ENFRIÁNDOSE **juntas**: cuentas cuya última op cae dentro de los últimos `DIAS_DORMIDA` días, o sea **cuánto del padrón sigue vivo** (separadas se miran en ANÁLISIS COMERCIAL, que es donde viven). Es el MISMO semáforo (`estado_comercial()` sobre los días desde la ÚLTIMA op; umbrales ÚNICOS en `comercial_sql.DIAS_ACTIVA`/`DIAS_DORMIDA`, que viajan en la respuesta para que el front no los hardcodee) y **NO es sinónimo de «cuentas operativas del mes»**: CTAS OPS mira una VENTANA de calendario y el semáforo mira cuánto hace que la cuenta no aparece — una que operó el 2 del mes pasado **sigue contando** y **no** es operativa de este mes. Por eso los rótulos llevan la unidad (mes / año / días) y las tres barras contables conviven. **DORMIDA/NUEVA no se pueden distinguir sin recorrer el histórico** (eso lo hace Análisis Comercial) y acá el scan se acota a `[corte − DIAS_DORMIDA, corte]`, así que no se muestran — inventar la diferencia sería peor que no darla. ⚠️ **El piso del scan de Q4 es `min(1-ene, corte − DIAS_DORMIDA)`**: con un corte de enero, una cuenta cuya última op fue en diciembre está ENFRIÁNDOSE y con el piso viejo el filtro la contaba y la tabla no la mostraba. Y el modal se abre con `ventana=reciente` cuando la fila entró por el semáforo: con `mes` saldría vacío y la pantalla desmentiría a su propio filtro | Ninguna |
...
| **PNL TÍTULOS** | Split 50/50: izq posiciones por ticker (cost-basis weighted-average) con KPIs VALOR ACTUAL y PNL NO REALIZADO; der detalle del ticker (KPIs COSTO/VALOR/PNL/NO REAL/COBROS/GAN % + **boletos del stock actual** con compras/ventas/neto y breakdown del PnL pasivo; los pseudo-boletos de ajuste van resaltados en ámbar). La tabla de boletos tiene **dos lecturas del mismo listado**: **DETALLE** (cronológico, el de siempre) y **CONSOLIDADO** —COMPRAS → VENTAS → un grupo por tipo de cobro (Interest payment, Amortization…) → AJUSTES, cada grupo con subtotal de cantidad, precio promedio ponderado e importe, y fila **NETO** abajo de todo. En CONSOLIDADO los grupos arrancan **cerrados** (se abren clickeándolos): la respuesta «cuánto compré, vendí y cobré» queda en 3 o 4 líneas. ⚠️ El subtotal de IMPORTE en moneda original sólo se muestra si el grupo NO mezcla monedas (si mezcla dice `MIX` y queda el IMP ARS, que viene pesificado boleto a boleto) | `GET /api/aum-pnl?id_cuenta=` → `/api/portfolio/pnl` | MONEDA · orden por columna · click en fila (toggle) · **DETALLE / CONSOLIDADO** en la tabla de boletos (el modo se mantiene al saltar de ticker) | Sólo lectura. 2 exports .xlsx (`exportarTodo`, `exportarBoletos` — este último con 2 hojas: Boletos + Consolidado). **Botón AJUSTES en la barra del shell** (admin) → modal `pnl-ajustes-modal.tsx` |
...
| **Contabilidad** | Resultado MENSUAL por título de las cuentas propias: **TENENCIA (RxT) + INTERMEDIACIÓN**, y el total es la **SUMA de los dos**. ⚠️ **NO HAY RENTAS** (regla del back office, 2026-09-02): cupones, dividendos y amortizaciones **no entran al informe** — no hay columna, no suman al total y `operaciones.negocio_movimientos` **ya no se lee**. Existió un tercer canal y se sacó ENTERO; media medida habría sido peor (un canal escondido que igual mueve el número del mes). ⚠️ **Lo que se compra y NO se vende no es resultado del mes** (regla del back office, 2026-09-01): su valuación final no entra en ningún canal — es el saldo inicial del mes siguiente, y recién ahí genera tenencia. (La valuación sale de `portafolio.tenencia` al último hábil de cada mes — misma foto que Tenencia Valorizada —; las compras/ventas de **`operaciones.movimientos_propias`** desde 2026-09-05. ⚠️ **CAMBIO DE FUENTE**: antes era `operaciones.operaciones` con `bruto` como importe. Ahora **una fila = una línea de TÍTULO** del feed de la cartera propia: se descartan las líneas cuya `unidad` es una moneda (ARS/USD/USDL/USDC), que son la pata de DINERO del mismo comprobante, y **la plata se toma de esa hermana** — el `total` de una línea de título son NOMINALES, no pesos (así lo trata `aunesa_negocio.agrupar_boletos`), y leer la hermana además trae la MONEDA de verdad en vez de obligar a adivinar el divisor por cartera de un bono que cotiza en paridad. La punta la da la `categoria` del feed con las MISMAS constantes de siempre, y las **solicitudes de FCI no entran a ninguna** — ahí muere el doble conteo que antes resolvía `emparejar_provisional_final` leyendo un campo que esta tabla no tiene. El plazo sale de la columna `plazo`. La clave del título es `unidad_to_match` **con fallback al `ticker` parseado**, el mismo espacio de claves que usa el motor de PnL sobre estos mismos boletos: una unidad que el catálogo todavía no conoce cae en su ticker en vez de quedar huérfana. ⚠️ **TERCERA DIRECCIÓN — `ajuste`**: línea de título que mueve CANTIDAD y no lleva plata (canje, split, amortización, rebautizo de especie). Son VÁLIDOS y son lo que la fuente anterior NO traía: sin ellos el cuadre fallaba y el título se iba a «sin conciliar» teniendo la tenencia toda la razón. Van como dirección propia, JAMÁS como una compra de importe 0 — eso ensuciaría `px_compra = compras/qty_compras` y el RxT saldría mal sin que nada falle. Y por eso conviven **DOS netos**: `neto_precio` (solo compras/ventas) topea lo que se reclasifica entre canales —valuar exige un precio y un ajuste no tiene ninguno— y `neto_cuadre` (todo lo que movió posición) es el que decide si la foto está explicada; confundirlos INVENTA plata, medido: una venta de 33.000 excluida a mano mostraba −30.000 de pérdida porque el `costo_salida` se cobraba sin su ingreso. Un TRD —operación genérica cuyo texto no dice Compra ni Venta— se clasifica por el SIGNO del importe, la misma regla de `agrupar_boletos`.) **UN SOLO MOTOR — `libro()` — corre el FIFO una vez y de ahí salen los dos canales**: **TENENCIA** = la CADENA de la planilla del back office, reproducida columna por columna y **visible en la pantalla** (con C/D = nominales y monto del cierre anterior, E/F = los del actual): `G no entran en RxT = E − C` · `H misma tenencia mantenida = min(C, E)` · `I monto mes anterior = H × (D/C)` · `J monto mes actual = H × (F/E)` · `K RxT = J − I` · `L variación del período = K / I`. Los cinco pasos viajan en la fila: sin ellos el RxT es un número que hay que creer. Verificado contra la planilla real (Toronto Trust Balanceado, jun→jul: H 61.481.010,022326 · I 241.799.955,418016 · J 246.452.715,294486 · K 4.652.759,8764696 · L 1,92%) y congelado por test. ⚠️ Se intentó afinar H con los SOBREVIVIENTES del lote inicial según FIFO y NO sirve: vendiendo TODA la posición inicial y recomprando más (1.000 → 1.200) del lote inicial no sobrevive nada y la tenencia daba **0** teniendo más nominales al cierre que al principio — **si hay nominales al inicio Y al cierre, sí o sí hay resultado por tenencia, y eso no impide que además haya intermediación** (regla del back office). Lo que paga `min`: los nominales comprados a mitad de mes devengan rendimiento de mes completo, imprecisión que la planilla acepta a cambio de no tener ceros absurdos; **INTERMEDIACIÓN** = `ventas − compras`, **menos el activo que SALIÓ** (a su precio del cierre anterior) y **sin el costo de lo que quedó en cartera** (ese se lo lleva la tenencia). ⚠️ **LA VALUACIÓN DEPENDE DE LA SITUACIÓN — no hay una sola fórmula**, y el invariante que las une es que `TENENCIA + INTERMEDIACIÓN + RENTAS` da **la plata real del mes** (`v_fin − v_ini + ventas − compras + rentas`) en los OCHO casos: quieta · intradía · alta · baja · achicó · agrandó · rotó · vendió-todo-y-recompró (congelado por `test_los_ocho_casos_dan_la_plata_real`). La TENENCIA suma además, **si la posición creció**, el valor al cierre de lo NUEVO menos lo que costó — la valuación de cierre es la foto final y no se toca, ya lo tiene todo implícito. Lo que se reclasifica está **topeado por lo que los boletos explican EN NETO**: ante un descuadre no se valúa, se marca. ⚠️ **Sin el término de SALIDA el informe INFLA**: vender algo que ya se tenía contaba la venta entera como ganancia — AO29 08/26 mostraba **1.869.729** donde había **−348.892** (el error, 2.218.620, era exactamente los 1.545 nominales que salieron × 1.436 del cierre anterior), y una venta SENEBI llegó a mostrar **6.465 millones** de PnL. ⚠️ **Hasta 2026-09-01 el total lo definía la IDENTIDAD `ΔValuación + ventas − compras + rentas` y la intermediación era el RESIDUO** (`total − rxt − rentas`) — dos fallas, las dos medidas sobre **AO29 08/26**: (1) la ganancia NO REALIZADA de lo comprado en el mes entraba como resultado — la cuenta arrancó sin AO29, compró y vendió (realizado **1.171.799**) y quedó con 955.084 nominales valuados en 1.324.988.033, y el informe cantaba **1.325.814.415 de intermediación**: la valuación de una posición nueva sumada como plata ganada; (2) ser el residuo hacía que la intermediación **absorbiera cualquier error de datos** (esos 955.084 nominales no los explica ningún boleto: el cuadre lo marcaba con un ⚠ y la fila igual sumaba al total). Y **el resumen se contradecía con su propio modal** — la fila derivaba la intermediación de la identidad y el modal la costeaba por FIFO: 1.325.814.415 contra 1.171.799, sin árbitro (REGLA #9).  Cada fila trae el **CUADRE de nominales** (fin − ini − Δ boletos): si no da 0 faltan boletos o hubo evento corporativo, y la fila lo marca en vez de mostrar un número sano que no lo es. Click en fila → modal ANCHO (1500px, sin textos explicativos — solo tabla y ⚠ con tooltip) con el **LIBRO del MES**: arranca con una fila de **POSICIÓN INICIAL** (nominales + valuación del cierre anterior, que entra como primer lote) y sigue con los boletos del período; cada fila lleva **NOMINALES ACUM.** y **PNL ACUM.** (realizado por costeo **FIFO** + rentas, corrido) con fondo resaltado — vender el 100% muestra `venta − valuación inicial`. **El `pnl_acum` de la última fila del modal ES la intermediación + rentas de la fila del resumen, por construcción** (misma pasada de `libro()`), y un test lo congela. Una venta que excede saldo inicial + compras se marca `sin_costo` — su intermediación queda INCOMPLETA, y eso ahora se declara en la fila (⚠ al lado del número) y en la barra (`sin_costo` en los totales); la columna PRECIO se eliminó (la fuente no la trae). La punta del boleto la da el catálogo `tipos_operacion` y, si no resuelve, el descriptor de Aunesa («… - Venta»); cauciones/futuros/opciones jamás. **El corte de mes es por LIQUIDACIÓN, no por concertación** (`pertenece_al_mes`): la tenencia es una foto liquidada, así que el boleto pertenece al mes en que liquida = concertación + **plazo real de `condiciones` en hábiles** (`plazo_habiles`: «24hs» 1, «48hs» 2, «3 días» 3, contado 0 — hasta 2026-09-04 solo se entendía «24» y ~7% de los boletos propios, medido, caían mal). ⚠️ **FCI vía Aunesa = DOS boletos por movimiento (provisional + final)** y se juntan en UNA fila (`emparejar_provisional_final`): ficha = fondo + punta (NO la cantidad — medido 2026-09-04: en suscripción el provisional trae cantidad 0 y el final las cuotapartes; en rescate el provisional trae la pedida y el final la liquidada, distinta), pareja fechada en el provisional, cantidad del final en suscripción y del provisional en rescate, importe del final. Lo que no empareja viaja tal cual. Borrar el provisional a ciegas rompía cuando el final caía en otro mes. ⚠️ **PARTIDAS SIN CONCILIAR — la TENENCIA manda** (regla del user, 2026-09-04): una fila cuyo cuadre no da 0 (`sin_conciliar` en la respuesta) se lista en un bloque aparte DEBAJO del total, atenuada, y **NO suma al total del mes**; la barra canta `⚠ n sin conciliar · $X afuera` (`totales.descuadres`, `totales.sin_conciliar_total`) y el Excel las lleva en su propia hoja. Se eligió esto (y no valuar el residuo a la foto) porque medido en 08/26 el residuo lo dominaban errores de datos — doble conteo FCI, plazo mal leído, rebautizo de unidades — y ponerle precio los convertía en resultado contable. DERIVADOS quedan afuera del cierre (son contratos, no nominales). Diag: `scripts/diag_conciliacion_contabilidad <mes> [--cuenta] [--out archivo]` (parejas FCI vs tenencia · kardex diario por clave · inventario de tipos · grafías de `condiciones`). Las BAJAS del período, que la planilla dejaba sin valuar, salen con número. Las **ALTAS PURAS** (0 nominales al cierre anterior y solo compras en el mes = comprado para dejar en cartera) NO son resultado de este mes: quedan fuera de la tabla y de los totales (2026-09-01 el bloque visual se quitó a pedido del back office; sobreviven como segunda hoja del Excel) — su resultado entra al RxT del mes siguiente; un alta que además vendió o cobró renta sí queda en el informe **La TABLA va CENTRADA y de UN SOLO COLOR** (el verde/rojo por signo pintaba media pantalla; queda el rojo SOLO para lo negativo, que en un informe contable no se puede leer igual que una ganancia), **la columna RENTAS se sacó** (venía entera en `—`; las rentas siguen sumando al total y la barra las canta si alguna vez hay una, para que no queden escondidas adentro del TOTAL), **el TÍTULO va a la izquierda y TOPEADO en 13rem**, cortando con «…» y el nombre completo en el tooltip — un asset sin ticker en el catálogo cae al display de la UNIDAD entera («[59419] SXC60 - ON MERCADO PAGO SERVICIOS DE PROCESAMIENTO CL.6 18/05/27 $») y UNA sola fila así abría la columna y dejaba medio ancho de página en blanco —, hay **cuatro líneas que cierran cada grupo** (título · nominales · valuación · compras+ventas), en `--t-border-2` y no en `--t-border`: ese es el mismo color del borde de cada fila y las líneas no se veían y **los encabezados llevan el MES**: `Nominales 07/26` / `Nominales 08/26`, `Valuación 07/26` / `Valuación 08/26` — así se lee cuál es la foto inicial y cuál la final sin adivinar. El Excel usa los mismos encabezados. | `/contabilidad/resumen?id_cuenta&mes`, `/contabilidad/detalle?id_cuenta&mes&key`, `/contabilidad/cuentas` + `POST`/`DELETE /contabilidad/cuentas` | `mes` (YYYY-MM, def mes pasado), selector de cuenta (las del ABM, `operaciones.contabilidad_cuentas`) — ⚠️ **las cuentas UNIFICADAS salen como UNA sola, rotulada «100 / 255»** (2026-09-05): Aunesa parte una cuenta de la casa en dos `id_cuenta` y para el negocio es UNA. Tratarlas separadas no da error, da **dos informes con la mitad de la historia cada uno** — la tenencia arranca partida, los boletos de una no explican los nominales de la otra y el cuadre chilla en las dos filas teniendo todo bien. La lista vive en **`config.CUENTAS_UNIFICADAS`** (es un hecho del negocio, no de esta pantalla), el PRIMER id del grupo es el CANÓNICO —con el que se escribe una exclusión o una fila del ABM, y el que viaja en la respuesta— y **toda query del módulo filtra por `id_cuenta = ANY(ids)`**: una que filtre por un solo id ve media cuenta y no falla. El router normaliza a la canónica ANTES de entrar porque `resumen` está cacheado por argumento (pedir «100» y «255» calcularía dos veces lo mismo). El front NO arma el texto: lo manda el backend en `display`. | **ABM de cuentas del proceso** (agregar/quitar/etiquetar): allowlist de Tesorería + admin (`require_escritura_tesoreria`, mismo gate que Interbanking). ⚠️ **ACOTADO (2026-09-05)**: solo se pueden elegir cuentas **CON movimientos en `operaciones.movimientos_propias`**, que es la fuente del informe. La LISTA la sigue eligiendo la mesa (hay cuentas con movimientos propios que no son de este proceso) — lo que cambió es que el id ya no se tipea a ciegas: el GET devuelve `elegibles` y el modal lo ofrece como desplegable, y el backend **valida** (`ok:false` con el motivo, que el modal muestra). Antes era texto libre: un id mal tipeado entraba, el informe salía VACÍO y eso se veía idéntico a un mes sin actividad. **La otra escritura es EXCLUIR movimientos**: desde el modal de una fila, una casilla por movimiento saca un boleto del RESULTADO del mes (`POST /contabilidad/excluir` · `/incluir`, `operaciones.contabilidad_excluidos`, mismo gate `require_escritura_tesoreria`, firmado con quién y cuándo). ⚠️ **SACA LA PLATA, NO EL HECHO**: sus nominales siguen contando para el cuadre — si salieran de las dos cosas, tildar una casilla rompería el cuadre del título y la fila entera se caería a «sin conciliar», o sea que sacar UN movimiento borraría el título completo del informe. La barra declara `n fuera del mes (altas) · $X al cierre` —⚠️ **hasta 2026-09-05 las ALTAS PURAS salían de la tabla sin dejar un número en la pantalla**: sobrevivían solo como hoja del Excel, que es el último lugar donde alguien mira cuando le falta una fila, y quien la buscaba no podía distinguir «no es de este mes» de «se rompió algo»—, `✎ n excluidos · $X afuera`, `n ajustes` y `⚠ n exclusión(es) sin efecto` (las que apuntan a una línea que Aunesa corrigió y cambió de hash: dejan de aplicar y se cuentan en vez de desaparecer). El cálculo no persiste nada |
| **Interbanking** | Los bancos de ACA, para CONCILIAR. **UNA sola vista** (ya no hay sub-tabs) y **UN día**: el selector es una sola fecha; sin fecha el backend usa el **día HÁBIL ANTERIOR a hoy** — el que está cerrado, con extracto y saldo final ya informados. **CONSOLIDADO BANCOS**: UNA fila por cuenta agrupadas bajo el **nombre del banco como título**, con **SALDO AL CIERRE · GASTOS BANCARIOS · IVA · IVAPERCEP · IIBBPERCEP · COM.TRANSF · OTROS IMP** (saldo al inicio, variación y movs. se sacaron el 2026-08-18: no se usaban). Las columnas del desglose las declara el BACKEND, no el front, y **las edita el EQUIPO** (botón DESGLOSE del modal): son un catálogo en la base (`bancos.gastos_baldes` + `gastos_balde_matchers`), no una constante; el **ORDEN se mueve con ▲▼** y es lo único que decide los empates — así se resolvió el pisón real de `IVA PERCEPCION RESOL GRAL`, que llega con el concepto en `IVA`: se sube IVAPERCEP y se le da un matcher por descripción, sin excepciones hardcodeadas — antes sumar la grafía que usa un banco nuevo era un commit y un deploy. **OTROS IMP = solo las 4 descripciones declaradas**, así que las columnas pueden no sumar el total; lo que no cae en ningún balde va a `resto` y el MODAL lo muestra como **MOVIMIENTOS RESTANTES** en vez de esconderlo. En el MODAL el desglose se abre COMPLETO en horizontal, sin contador de movimientos, con GASTOS BANCARIOS separado por una línea de sus partes («= suma de»). La barra tiene **MOVIMIENTOS A CONCILIAR** (`bancos.conciliacion_pendientes`: lo confirmado en CONCILIAR que hay que arreglar en HYGIRUS — cada fila dice QUÉ HACER, «cargar en el mayor» o «sacar del mayor», con la descripción **tal como viene de SU lado** para que sea encontrable allá; sin filtro de fecha, lo resuelto se marca y no se borra), **CONCILIAR** (nuestro saldo al cierre contra el ÚLTIMO saldo del mayor contable que el usuario sube como Excel; el navegador solo abre el archivo y manda la grilla cruda, todo el criterio vive en el backend. ⚠️ La `D`/`A` del saldo es **formato de celda** (`#,##0.00" D";#,##0.00" A"`), o sea que `A` = negativo — medido sobre el export real. Si no concilia, busca qué movimientos del día suman exactamente la diferencia; si coincide al invertir el signo, lo avisa y NO lo corrige. `POST /conciliar`, no persiste nada), **DIFERENCIAS BANCARIAS** (¿la variación del saldo está explicada por sus movimientos? `cierre(hoy) − cierre(día anterior) == Σ movimientos de hoy`; lo que sobra casi siempre es el banco registrando un movimiento con fecha de anteayer que impacta en el saldo de ayer — y como `Σ movimientos = cierre − apertura`, esa diferencia **ES** el salto entre el cierre de un día y la apertura del siguiente, que se muestra al lado como evidencia. Se concilia contra el BANCO: los manuales no entran y los ignorados sí; sin alguno de los dos cierres dice «sin dato» en vez de inventar una diferencia. `GET /diferencias`, solo lectura), un **filtro por BANCO** (client-side sobre lo que ya trajo el consolidado; alcanza también al REPORTE FIN DE DÍA pero no al alta de manuales), **REGISTRAR MOVIMIENTOS MANUALES** (bancos que no están en Interbanking + movimientos que el banco no informa: `bancos.cuentas.origen='manual'` y `bancos.movimientos_manuales`; **siempre impactan el saldo al cierre** del día que muestra la vista —en una cuenta real se suman a su extracto, en una manual son todo el saldo—, el día y la moneda NO se eligen y el selector de cuenta se llena con las del banco elegido; el job no las puede pisar porque recorre lo que le devuelve Interbanking), **REPORTE FIN DE DÍA** (el saldo al cierre de todas las cuentas: **una tabla POR BANCO** —se probaron las dos matrices y las dos quedaban con el 90% de las celdas vacías, porque cada cuenta pertenece a UN banco— empaquetadas hasta 20 filas de alto y 15 columnas de ancho, con espacios grandes entre tablas, ARS primero adentro de cada banco la cabecera azul con el logo UNA vez arriba de todo y la firma «Hecho en ACAQuant» en chico, y botón **COPIAR IMAGEN** que DIBUJA el PNG de cero en un canvas —no es una captura: por eso la UI no se cuela, sale siempre en claro para el mail y no depende del tamaño de pantalla; si el navegador no deja copiar imágenes, la descarga). **Clic en cualquier número del desglose = AUDITOR**: la tabla queda mostrando SOLO las filas que lo componen, con una barra que dice cuántas son y cuánto suman (la suma la calcula la pantalla sobre lo visible: si no coincide con el número clickeado, el desglose y el detalle se contradicen). El DESCARGAR respeta el filtro. El copiar de esos valores se mudó a un ícono ⧉ al lado de la etiqueta. La cuenta va a la izquierda con el **número ENTERO**; las columnas de datos van centradas, de ancho parejo y separadas por una línea. **NO hay subtotales por banco ni totales por moneda** (los sacó el back office). **El CIERRE tiene DOS fuentes y la celda rotula cuál**: sin rótulo = EXTRACTO; **«saldo»** = lo informa el banco pero la cuenta no se movió y no hay extracto que lo respalde; **«≠»** = las dos no coinciden, con los dos números y la diferencia en el tooltip — y ese badge **es un BOTÓN: abre el selector QUÉ SALDO VALE** (user, 2026-09-03: «no es lineal, hay veces que vale uno y otras que vale otro»). El modal muestra los dos candidatos lado a lado (informado / cierre del extracto), cuál está **en uso** y cuál fue **elegido**, con «volver al automático»; la elección se guarda **por cuenta y por fecha** (`bancos.fuente_elegida`, `PUT /saldo/fuente`, allowlist de Tesorería + admin) y el backend **re-sella el cierre**, así que arrastra el SALDO AL INICIO del día siguiente, CONCILIAR y DIFERENCIAS. Elegir **no tapa el hallazgo**: el ≠ sigue ahí (pasa a decir «≠ elegido»). Sin ninguna va «—», nunca 0. **GASTOS BANCARIOS lo DERIVA el backend** de las reglas + marcas; sin una sola regla ni marca cargada viene `null` — «—» y jamás cero («no sabemos» ≠ «no hubo gastos»). ⚠️ **La fila tiene DOS gestos**: clic en la CUENTA abre el **MODAL de movimientos del día** (reemplazó a la sub-tab «Detalle por cuenta», que obligaba a elegir banco y cuenta en dos selectores para ver una fila que ya estabas mirando); clic en una celda de DATOS **copia el valor al portapapeles**. El **MODAL** trae la cabecera de la cuenta, el día según el banco (apertura · créditos · débitos · cierre · neto · movimientos) con las alertas **NO CIERRA** / **INCOMPLETO**, la tabla de MOVIMIENTOS con 8 columnas (FECHA · DESCRIPCIÓN · CONCEPTO · COD OP · COD OP BCO · COMPROBANTE · SUCURSAL · IMPORTE) más **GASTO** y **CUENTA**, con la **DESCRIPCIÓN llevándose el sobrante de ancho** para que salga entera (si igual se lee cortada, el corte lo hizo el banco: llega truncada a ~25 chars), los botones **REGLAS** y **DESGLOSE**, y un botón **DESCARGAR** (.xlsx client-side vía `lib/xlsx-export`, con el importe FIRMADO para que la columna sume el neto en Excel). Cierra con Esc, con la ✕ o clickeando el fondo. La CONTRAPARTE no es columna propia: viene en el **13%** de los movimientos (medido) y va debajo de la descripción. La **hora** se dibuja solo si no es `00:00:00` — medido sobre 178 movimientos, Interbanking **no informa la hora**. Lee de `bancos.*`, que llena `jobs/interbanking_sync` cada 2hs de 9 a 19 ART (día hábil anterior + hoy: extracto + saldo por cuenta) y que **retiene solo 3 fechas**. **La vista NUNCA le pega a Interbanking** (el límite de 100 llamadas/minuto es del ABONADO); la barra muestra **«Última actualización DD/MM/AAAA HH:MM»** y **quién más tiene la vista abierta** (`bancos.presencia`, el poll de 60s ES el heartbeat, TTL 180s). **GASTOS BANCARIOS** se clasifica en DOS capas: **reglas** (catálogo `bancos.gastos_reglas`, campo+operador+valor, ABM en modal desde el botón REGLAS) y **marca manual por movimiento** (`bancos.gastos_overrides`, columna GASTO del modal, un clic) — **la marca manual GANA sobre la regla** y la celda muestra de dónde salió (subrayado = manual, con ↺ para volver a la regla). Se deriva en la LECTURA, no se materializa. Sin ninguna regla ni marca la columna va «—», no 0. **IGNORAR un movimiento** (`bancos.movimientos_ignorados`, columna CUENTA del modal) es OTRA pregunta que GASTO: GASTO dice *qué es*, CUENTA dice *si suma* — un duplicado del banco sigue siendo un gasto, lo que no es es DOS gastos. La fila NO se borra: queda **tachada** con quién/cuándo/motivo y afuera de los gastos, del desglose y de la suma del auditor (que los cuenta aparte); los créditos/débitos del día NO se tocan — esa es la aritmética del extracto. **Ignorar gana sobre la marca manual.** Mismo modelo de PUROS OVERRIDES que `tesoreria_exclusiones`, con `ON DELETE CASCADE` sobre la retención de 3 fechas. Escritura: allowlist `operaciones.tesoreria_escritores` + admin, auditada en `bancos.gastos_audit`. **NO se mezcla con Tesorería** — objetos sin clave en común; ver `docs/INTERBANKING.md`. El **número de cuenta va ENTERO**; el **CBU no sale nunca** (hay tests). Cada lectura queda auditada en `bancos.audit_lecturas`. Poll 60s | **`GET /interbanking/consolidado`** (la vista) y **`GET /interbanking/vista?cuenta_id&fecha`** (el modal) + `/cuentas` | **`fecha`** (una sola; futura → 400), `cuenta_id` en el modal | **Solo la clasificación de gastos**: `POST /manual/cuentas`, `DELETE /manual/cuentas/{id}`, `POST /manual/movimientos`, `DELETE /manual/movimientos/{id}`, `POST /gastos/reglas`, `DELETE /gastos/reglas/{id}`, `PUT /gastos/movimiento`, `PUT /gastos/ignorar`, `POST /gastos/desglose`, `POST /gastos/desglose/orden`, `DELETE /gastos/desglose/{clave}`, `POST /gastos/desglose/matchers`, `DELETE /gastos/desglose/matchers/{id}` — sobre tablas NUESTRAS, con allowlist y auditadas; un test las ENUMERA. **Hacia Interbanking no se escribe nunca** (`core/interbanking.py` solo hace GET, congelado por test) y el proxy de Next solo deja pasar escrituras bajo `/gastos/*` y `/manual/*` |
...
| **Custodia** | **LO QUE LA CAJA DE VALORES (CVSA) TIENE REGISTRADO** — otra FUENTE, no otra vista: todo lo demás del sistema sale de Aunesa (el back-office tercerizado) y esto es lo que la Caja registra; cuando difieren, la razón legal es de la Caja. **Dos sub-tabs**. **① TENENCIAS** — qué hay hoy y en qué difiere de Aunesa. Sale de `portafolio.custodia_cvsa`. Chips por ESPACIO DE NUMERACIÓN (LIQUIDADORA / GARANTÍAS / ⚠ desconocido): CVSA usa **tres** espacios para el mismo agente —`74` comitentes, `70074` liquidadoras, `80074` garantías— y el número de la derecha SE REPITE entre ellos, así que cada cuenta se muestra con su par completo y su etiqueta. Columnas: CUENTA · DENOMINACIÓN · INSTRUMENTO · CVSA · ESTADO · **VN AUNESA** (`cantidad − gar_cantidad`) · **VN BYMA** · **DIFERENCIA**. Botonera **T0 / CIERRE T-1**: contra qué se compara Aunesa — es lo ÚNICO que viaja al servidor; BYMA actualiza DESPUÉS DE LAS 21, así que durante el día comparar contra T0 marca desfasaje como si fuera descalce (el botón T0 se pone ámbar y lo explica en su tooltip). **② MOVIMIENTOS** — qué se liquidó. Sale de `portafolio.custodia_movimientos`. ⚠️ **UNA FILA = UN MOVIMIENTO, NO UNA PATA**: BYMA informa por PARTIDA DOBLE (cada `instructionReference` viene dos veces, con volumen de signo opuesto, una por cuenta) y el plegado a `ENTREGA → RECIBE` lo hace el BACKEND. Columnas: LIQ. · INSTRUMENTO · CVSA · ENTREGA · RECIBE (con la etiqueta de qué cuenta es: `222222222` sin etiqueta se lee como un comitente y es la Cta. Gtías. House) · VOLUMEN · MONTO · MON. · ESTADO · REFERENCIA. Ventana **DÍA / 7D / 30D** (el feed corre varias veces al día: mirar solo hoy a las 9 devuelve vacío y eso no significa nada). Chips: DESCALCES (alerta) · **SIN PAR** (informativo: contra otro agente solo vemos nuestra pata) · SIN INSTRUMENTO. ESTADO queda en `—` hasta que se habilite la consulta por referencia (necesita que el Droplet llegue a BYMA). **Dos cosas que no se ven en ningún otro lado**: (1) **qué está TRABADO** — `subBalanceType` separa AVAILABLE de EMBARGO / BLOCKED_FOR_PLEDGE / PENDING_REDEMPTION…, detalle que Aunesa no da; (2) **qué no sabemos nombrar** — CVSA identifica los papeles con un número propio (`cvsa_id`) y la traducción sale de `assets.codigo_cnv`: cuando falta, la fila se muestra igual con su número crudo. **Los datos los sube la PC de oficina** (`scripts/byma_feed.py` → `/api/ingest/custodia/*`): las APIs de BYMA están detrás de AppGate y el Droplet no las alcanza. **UN request por tab y los filtros se aplican en el cliente** — la foto de un día es un conjunto cerrado, así que filtrar en memoria es instantáneo y los contadores no pueden contradecir a la lista (antes cada chip disparaba tres queries y trababa la vista). Poll 5 min con la antigüedad de la foto siempre a la vista: una lista vacía con el feed parado no es «no hay tenencia», es «no sabemos» | `/back-office/custodia/tenencias` · `/back-office/custodia/movimientos` | tenencias: `fecha` · `fuente` (t0\|cierre) — movimientos: `fecha` · `dias` (1-30) | Ninguna (solo lectura) |
| **Comisiones FCI** | Lo que cobra ACA Valores por la TENENCIA de fondos, sin planilla: tabla por fondo (**AuM** = valuación en la foto de corte · último día · acumulado del mes; cada fila se abre y muestra las cuentas que la componen), acumulado mensual histórico (barras ARS/USD separadas) y acumulado por sociedad gerente. La gerente es clickeable: al elegirla, el gráfico, los totales, fondos y detalle se recalculan en ese contexto; volver a elegirla la limpia. Fórmula `valuación × fee_admin ÷ 2 ÷ 365` devengada por día CORRIDO (cada foto cubre hasta la siguiente: el viernes devenga 3 días). Un fondo sin `fee_admin` NO vale cero: se marca `sin fee` y se cuenta aparte. Modal `?` con la fórmula y el fee de cada fondo/gerente. **Todo el cálculo vive en `api/services/comisiones_fci.py`; el front no deriva nada.** | `GET /comisiones-fci?mes&operador&nivel_1&nivel_2&nivel_3&gerente` · `/detalle` (mismo contexto) · `/serie` (mismo contexto) · `/filtros` · `/meses` · `/fees` | Mes (solo meses con foto); Operador; Nivel 1/2/3 multi (OR dentro de un nivel, AND entre niveles); gerente por click | — (solo lectura) |
...
### `scanner.py` — `/api/scanner`, módulo `renta-variable` (5)
GET `/cedears` · `/ccl` · `/pivot/{ticker}` · `/day-trading` **(admin)** · `/companeros/{ticker}` **(admin)**
(`/returns` y `/quant` se borraron el 2026-09-10)
— **ninguno escribe**

### `trading.py` — `/api/trading`, módulo `trading` (6)
GET `/pivots` · `/intraday` · `/pivot-radar` · `/universo` · `/monitor/universo` · `/monitor`
— **ninguno escribe**

(`/trades`, `/renta-fija` y `/adr-zonas` se borraron el 2026-09-01 con el refactor de la vista;
el router `estrategia.py` desapareció entero en el mismo cambio. `/pnl-historico` (GET+POST) se
borró el 2026-09-04 con su tab: era la ÚNICA escritura de toda la vista TRADING.)
