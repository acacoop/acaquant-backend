...
from agente.alta import INFO, NO_SE, OK, REVISAR
from agente.reloj import en_rueda as _rueda
...
def explicar(ticker: str) -> dict:
    ...
    try:
        ...
        logger.exception("agente/pata: no se pudo leer la base: %s", e)
    ...
    if not sembradas and not en_primary and base:
        try:
            ...
            logger.warning("agente/pata: no pude emparejar por ficha (%s)", e)
    if por_ficha:
    ...
    if elegida:
        try:
            ...
            logger.warning("agente/pata: sin estado de suscripción (%s)", e)
