...
- **`tasa: null` = sin dato, se muestra vacío** (nunca `0`, que sería una tasa
  ...
  cuántas filas resolvieron. La cobertura se midió en prod (diag ya borrado).
...
| 2026-09-10 | **feat(renta-fija):** `GET /api/cotizaciones/bono/{ticker}` y `GET /api/analitica/simular-inversion` suman **`acumulado`** en cada flujo: lo cobrado hasta ese pago inclusive (en el bono, sumando solo los pagos FUTUROS; los vencidos van en `null`; en el simulador, en montos escalados al importe). Lo muestra la columna ACUMULADO de la tabla del cronograma en los dos modales, que reemplazó al gráfico del flujo. Aditivo, nada se quita. |
| 2026-09-08 | **wipe(manager):** se borra el sub-tab `MANAGER → TÍTULOS → BONOS` y con él el router `/api/manager/ons` **completo** (`GET /ons`, `/ons/values`, `/ons/conciliar`, `/ons/ignoradas`, `POST /ons`, `PATCH /ons/sector`, `DELETE /ons`, `POST /ons/parse-flujos`, `POST/DELETE /ons/ignorar`) y casi todo `/api/manager/bonos` (`GET /bonos`, `/bonos/sin-tasa`, `POST /bonos`, `POST /bonos/parse-flujos`, `DELETE /bonos`). **Sobrevive `GET /bonos/sin-flujo`**, que consume el check «Títulos sin flujo» de VALIDACIONES. Lo que hacía esa pantalla lo hace EL AV AGENT (`bono_sin_flujo`, `bono_sin_tasa`, `soberanos_faltantes`, `on_faltante` con `alta_bono`/`alta_flujos`/`alta_on`). Las PUERTAS de escritura (`bonos_admin.upsert_bono`, `ons.upsert_on`) **no se tocan**: son por donde escribe el agente. Quedan sin pantalla, a propósito: cargar a mano un bono que 1816 no publica, restaurar una ON descartada, y reclasificar el sector de una ON (ver `AGENT.md` §0.eo). **531 → 516 endpoints.** |
