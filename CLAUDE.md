...
> **2026-08-19 — EL AGENTE PERSISTE** (§0.t): `permiso_flojo`, `tabla_quieta` y
> `db_cambio` corrían de noche y **solo imprimían en el log** — la tab SKILLS
> decía «se ve en ENCONTRÓ» y no se veía en ningún lado. Ahora se persisten con
> `alcance='sistema'` (de REEMPLAZO, como `live` → **excluido del
> `max(corrida_at)`** vía `av_agent.ALCANCES_VIVOS`, si no la relevada nocturna
> desaparece de la pantalla en silencio). Cada detector declara EN QUÉ JOB corre
> y un test exige que ese job **escriba** hallazgos; el horario se lee del
...
> detección de endpoints **peores que SU PROPIA normalidad** (no un ranking de
> lentos, que es lo que no funcionó). La tab LATENCIA de Manager se eliminó; la
> tabla y el middleware quedan.
>
...
> front** (§0.l) — el motor sigue, la puerta es el agente. Regla que quedó de eso:
> **una tarea de IA existe solo si alguien lee su salida** (de 11 quedaron 4).
...
- `manager` — `manager_users`, `role_matrix`, `role_audit`, `grupos`, `job_runs`, `pyrofex_instruments`/`pyrofex_discovery`; `latencia_endpoints` (telemetría de LATENCIA endpoint × hora — agregado que flushea el middleware `api/telemetria.py`. **La tab LATENCIA de Manager se eliminó el 2026-08-19**: un ranking muestra lo LENTO, no lo ANORMAL, y esa lista no cambia nunca. Ahora la lee el AV AGENT (`av_agent_latencia`), que compara cada endpoint contra SU PROPIA mediana y avisa solo cuando se degrada — la tabla y el middleware no se tocaron); `db_tamano`/`db_tamano_dia` (foto DIARIA del peso de cada tabla, `jobs/db_tamano` 23:30 UTC: guarda SOLO hoy y ayer y purga en el mismo INSERT, porque lo que informa es el DELTA —qué apareció, qué creció, qué desapareció— y no el tamaño; el corte se auto-calibra sobre el tamaño real de la base); `superficie_dia` (la MEMORIA de la superficie HTTP — cada ruta con su gate efectivo, SOLO hoy y ayer con purga en el mismo INSERT, escrita por `av_agent_seguridad.sacar_foto` en el mismo job: sin ella el chequeo de permisos solo sabe decir cuántos endpoints están abiertos hoy, con ella detecta el que apareció sin gate y el que **perdió** el que tenía, que un conteo no ve porque se cierra uno y se abre otro); la vieja `uso_modulos` fue ELIMINADA 2026-08-04). (`asistente_chats`/`asistente_mappings` quedaron HUÉRFANAS: el asistente de negocio se eliminó el 2026-08-19 — ver `AV_AGENT.md` §0.k. No se dropearon: borrar código se revierte, borrar datos no.)
- `home` — `market_quotes` (watchlist HOME), `news_headlines`.
- SALUD (**se ve SOLO desde el AV AGENT** desde 2026-08-19 — el panel de Manager, el botón de la barra y el modal de alertas se dieron de baja; el MOTOR `api/services/salud.py` no se tocó y el agente lo lee. Ver `AV_AGENT.md` §0.l): UN modelo de CHEQUEO que unifica toda la observabilidad (`api/services/salud.py`). Nace del incidente 2026-08-07 — el backfill de tenencias fallo dos dias y nadie se entero: no faltaban datos (job_runs, controles_datos, latencia, arbol de diagnostico, triage con IA) sino que estaban en seis pantallas y **ninguna respondia si el sistema estaba sano** (la card de AuM en VERDE con el job muerto hacia 48h). De un job importan TRES preguntas y solo se miraba la segunda: **corrio cuando debia** (la resta que nadie hacia), **salio bien**, **dejo el dato fresco**. Los chequeos se generan SOLOS: los jobs de `deploy/crontab.txt` via `jobs_catalogo` (un cron nuevo aparece sin tocar nada) cruzando el schedule contra el ultimo run; lo unico a mano son los CONTRATOS DE FRESCURA (`CONTRATOS` en el service, 5 tablas criticas — sumar una es UNA linea) porque ninguna maquina puede adivinar que `portafolio.tenencia` debe tener el ultimo dia habil. Un job sin instrumentar es **WARN, no verde**: no se puede afirmar que esta bien algo de lo que no se sabe nada. El estado actual NO se persiste (se evalua en vivo); si se guardan `manager.salud_eventos` (las TRANSICIONES, append-only, con la evidencia congelada — una vez que el job vuelve a correr el motivo ya no existe), `manager.salud_config` (toggle de alerta por chequeo; silenciar NO lo saca de la pantalla, solo deja de abrir el modal; lo no configurado ALERTA por default para que un chequeo nuevo avise sin darlo de alta) y `manager.salud_vistos`. La INTERRUPCION vive ahora en el modal del AV AGENT y conserva sus reglas: solo ante una transicion NUEVA a problema y sin ver, nunca cuando algo se arregla, y nunca en intervalo fijo (un modal que repite lo mismo se cierra sin leer). Endpoints: `POST /api/ia/av-agent/salud` (el diagnostico razonado), `/av-agent/salud/pendientes`, `/av-agent/salud/vistos`, `/av-agent/salud/silenciar`, `/av-agent/salud/recontrolar`. Un job con DOS lineas de cron es UN chequeo (se colapsan quedando el estado menos alarmante: dos crons son dos ventanas del mismo job).
- `agente` — **la MEMORIA del AV AGENT, schema propio desde 2026-08-22** (sus tablas nacieron en `mercado` por inercia del primer detector y se mudaron con `ALTER TABLE ... SET SCHEMA` — metadata, cero copia de datos; la mudanza vive en un bloque `DO` de `sql/schema.sql` que corre antes de los `CREATE TABLE`): `av_agent_items` (⭐ la CANÓNICA: un objeto por problema, PK `clave` = sujeto|causa), `av_agent_hallazgos` (foto por corrida → LA LISTA), `av_agent_centinela` + `av_agent_latido` (monitor en vivo → VIGILANCIA), `av_agent_evals` (eval set, append-only), `av_agent_runs` (diagnóstico masivo), `av_agent_avisos`/`av_agent_aviso_items`/`av_agent_preguntas` (lo que el agente DICE — comunicaciones, no problemas: `core/ciclo.TIPOS_COMUNICACION`), `av_agent_propuestas`, `av_agent_acciones`, `av_agent_trazas`, `av_agent_lecciones`, `av_agent_errores`, `av_agent_control`, `av_agent_ignorados`, `av_agent_seguimiento`. Lo de SALUD sigue en `manager` (`salud_*`, `controles_datos`, `superficie_dia`, `tabla_perfil`).
- `mcp` — `oauth_clients`/`oauth_codes`/`oauth_tokens` (TTL automático).
- `ia` — observabilidad del gateway de IA (`core/ai.py`, ver `docs/AV_AGENT.md`): `trazas` (cada llamada LLM: tarea, modelo, tokens, latencia, ok/error, feedback, detalle/respuesta/razonamiento, conv_id) y `config` (presupuestos editables desde Manager). Router HTTP: `api/routers/ia.py` (bearer + `require_module("ia")`): briefing (**determinista, no gasta un token**), observabilidad, presupuestos/saldo y **el AV AGENT** (`/api/ia/av-agent/*`, admin-only). Tambien `research` (mail diario 1816 via IMAP, `jobs/research_mail.py` — cuerpo crudo + destilado LLM + FTS español). **Solo 5 tareas de IA vivas**: `av_agent_informe`, `av_agent_accion`, `av_agent_error` (traduce UNA línea de log a castellano, una vez por PATRÓN y persistida en `agente.av_agent_errores`), `research_destilar`, `smoke`. Las tablas `triage_incidentes`/`triage_estado`/`calidad_flags` quedaron huerfanas (sus jobs se eliminaron el 2026-08-19, ver `AV_AGENT.md` §0.k).
...
- `aca` — **RESUMEN EJECUTIVO DE INVERSIONES** (vista `/aca` — **doc vivo: `docs/ACA.md`**). La cartera PROPIA de ACA contada para los gerentes: NO es live, es una **foto MENSUAL** que la mesa carga a mano. Criterio: *un Excel con las fórmulas ya puestas* — lo derivable se deriva SIEMPRE server-side (montos, ponderaciones, share, métricas, acumulados) y solo se tipea lo que ninguna fuente tiene (el **precio de corte del mes**, el VN, el MEP/A3500 del informe, los rendimientos externos). Tablas: `periodos` (cabecera: fecha del informe + MEP + A3500), `activos` (PK periodo+unidad — **solo inputs**: vn, px, override de monto, tasa, obs), `moneda_regla`, `emisor_destacado`/`clase_destacada` (catálogos de las métricas), `series` + `historico` (la planilla mensual), `audit`. **La FICHA del título NO se duplica**: cartera/emisor/calificación/clase/vencimiento/ticker se resuelven en cada lectura contra `portafolio.assets` (el maestro de Manager → Títulos) — copiarla habría creado una segunda verdad que se desincroniza sola, como ya mostró el rebautizo de especies de Aunesa. `monto = vn × px / 100` para carteras ARS/DL/HD (paridad) y `vn × px` para FCI/RV — **la misma regla de divisor que la valuación del AuM**; sin vn o sin px el monto es **null, no 0** ("sin cargar" ≠ "vale cero"). **Total Dolarizado / Total Pesos** salen de `moneda_regla`, editable en Manager → ACA, donde la regla de **CLASE gana sobre la de CARTERA**: eso es lo que parte el FCI por moneda (MM USD y HD T1 → dólares; MM ARS, ARS T1 y RENTA VARIABLE → pesos) **sin sacarlo de su cartera** — verificado contra la planilla, donde HD+DL da 83% y el Total Dolarizado del informe dice 92,7%, y la diferencia son exactamente los FCI en USD. Lo que ninguna regla ubica NO se reparte a dedo: cae en `sin_clasificar` y la vista lo canta. **Las carteras del informe se DERIVAN de lo cargado** (2026-08-19): las 4 canónicas ARS/DL/HD/FCI siempre y primero, y detrás cualquier otra que aparezca (DEUDORES, etc.), alfabética — antes era una constante cerrada y una cartera nueva sumaba al total pero no abría cuadro, sin forma de arreglarlo desde la app. `huerfanos` quedó SOLO para el título sin ficha. ⚠️ El DIVISOR no se deriva: fuera de ARS/DL/HD el monto es `vn × px` — si una cartera nueva cotiza en paridad hay que sumarla a `_DIVISOR_PARIDAD` (errarle es factor 100). **Acumulado del histórico = (1 + acum anterior) × (1 + mensual) − 1, y NO se persiste** (se deriva en la lectura, así no puede contradecir a sus insumos); un mes sin dato ARRASTRA el acumulado, no lo reinicia. **NADA se completa solo desde otra fuente** (2026-08-19, regla del user: *«nada de ACA tiene que ser automático… en el sentido de cargar datos solos de otras fuentes»*): el histórico tenía series que traían el mensual de `macro.series_macro` (el A3500 salía de la variación del dólar) y se dio de baja entera — service, catálogo, pantalla y el marcador `·a` de las dos vistas. `aca.series.fuente`/`escala` quedan VESTIGIALES (nadie las lee; `apply_schema` normaliza a `'manual'`). Lo DERIVADO sigue: deriva de lo cargado, no importa un dato. Congelado por test. Al deployar la columna A3500 queda vacía — `scripts/aca_congelar_automaticos` (one-shot, dry-run por default) persiste esos valores como carga manual. RBAC: lectura = módulo `aca` (rol **`empleado_aca`**) ∪ admin ∪ escritores; escritura = **la allowlist de Mesa de Dinero** (`operaciones.mesa_dinero_escritores`) + admin. **La tab `Manager → ACA` NO la da el umbrella `manager`** (2026-08-19): la da la CONJUNCIÓN de (acceso a Manager por cualquier módulo `manager*`) Y (escritura en ACA) — gate `_ACA`/`require_escritura_aca` en `api/routers/manager/__init__.py`, y en el nav la capacidad **`manager-aca`** que publica `/api/me` (**NO agregarla a `core.roles.MODULES`**: sería un checkbox en ROLES Y PERMISOS que no controla nada, igual que `mesa-dinero`). Existía porque pedir `manager` obligaba a hacer admin a un `asistente_comercial` que solo tenía que cargar el rendimiento del mes. `empleado_aca` es un rol NUEVO y no `sales` renombrado a propósito: `sales` es `DEFAULT_ROLE` y todo email nuevo cae ahí, así que colgarle `aca` habría dado la cartera de la casa a cualquier alta automática. **JAMÁS al portal invitado** (REGLA #8, congelado por test). La vista se sirve en UN request: `GET /api/aca/vista`.
...
evals/       # datasets de evaluación del programa de IA (ver docs/AV_AGENT.md)
