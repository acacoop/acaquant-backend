...
| **POSICIONES Y DIFERENCIAS** (`ap5-posiciones-view.tsx`) | El reporte que la mesa manda por mail ("RESUMEN POSICIONES AGRO Y DÓLAR FUTURO"), servido desde lo que dice **LA CÁMARA** (`ap5.portfolio`, A3/ACyRSA) y no desde nuestro registro de boletos. **Reemplazó a DIFERENCIAS DIARIAS** (2026-08-25), que leía el TEXTO de `negocio_movimientos`: eran los mismos pesos por dos caminos y cuando no coincidían no había forma de saber cuál mandaba. **DOS TABS** — FUTUROS AGRO · FUTUROS DÓLAR — y en cada una los dos lados del mail: **Cooperativas a la izquierda, MUNDO ACA a la derecha**, cada uno con su Top 10 + y Top 10 −. ⚠️ **El ranking va por ACUMULADO, no por la diferencia del día**, y se DERIVA de las mismas filas que dibuja la tabla de abajo (una sola query: si corriera la suya, la pantalla podría contradecirse a sí misma). Arriba y a lo ancho, las cards de "Diferencias ACA hoy" **partidas por moneda** (el agro liquida en Dólar MtR y el dólar futuro en Pesos: no se suman nunca). El bloque POR INSTRUMENTO muestra la posición en su UNIDAD (toneladas, dólares) y no en contratos — el multiplicador sale de `ap5.contratos`, y un símbolo sin multiplicador muestra los contratos crudos EN ROJO en vez de una cantidad 100× más chica que parece bien | `/api/ap5/vista` (**un solo request**: los bloques tienen que hablar de la MISMA fecha, o un job corriendo en el medio deja el resumen en un día y el ranking en otro), `/api/ap5/cuentas`, `/api/ap5/semilla` | Selector de **día** (solo días CON POSICIÓN — la cámara tiene su propio calendario y puede ir rezagada; el día anterior es el hábil previo con datos, no "ayer"), tab AGRO/DÓLAR. La barra declara los **faltantes** (símbolos sin multiplicador, cuentas sin nombre/grupo/semilla) en vez de esconderlos | **Sí**: click en una fila del acumulado abre el modal para cargar **grupo** (Cooperativas / MUNDO ACA — la cámara no lo sabe y NO se deduce del nombre, REGLA #9) y **semilla** del acumulado (el arrastre anterior a nuestra serie; la cámara manda la diferencia del día, no el arrastre). Las dos cosas las escribe una persona y el job **jamás las pisa** |
