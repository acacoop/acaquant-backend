...
| **Informe** | 4 cuadrantes de toda la mesa: **Q1** cuentas por segmento (modos `cuentas`/`op. mes`/`op. año`/`aranceles`; el % es **penetración sobre la base de ESE segmento**), **Q2** ranking volumen+aranceles por comercial, **Q3** aranceles por segmento, **Q4** detalle del segmento (clientes) | `/comercial/informe`, `/informe-segmento`, `/informe-aranceles-segmento`, `/informe-segmento-detalle`, `/informe-cliente-ops` (modal; param `ventana=mes|ano`) | Desde/Hasta, moneda, filtros madre (**el `operador` madre va SOLO al ranking Q2**; en Q1/Q3/Q4 `operador` es el drill-down del comercial clickeado). Click en comercial re-scopea Q1/Q3/Q4; click en segmento filtra Q4; **click en un cliente de Q4 → modal con SUS boletos del MES del corte** (`/informe-cliente-ops`: ventana y predicado idénticos a CTAS OPS y al filtro, así una fila que el filtro dejó pasar nunca se abre vacía). ⚠️ La tab OPERACIONES de Q4 **se dio de baja** (2026-08-31) y con ella la lista de boletos que viajaba en `/informe-segmento-detalle` — eran hasta 20.000 filas sin dueño, capeadas, que no contestaban ninguna pregunta; era el viaje más caro del endpoint. La tabla de clientes incluye ahora las cuentas que **operaron sin dejar arancel** (pill «operó», arancel en cero): sin ellas el contador del filtro daba MENOS que CTAS OPS —medido en pantalla, 30 contra 33— porque la lista nacía de un `arancel > 0` y el contador de arriba no; **filtro de Q4 con TRES estados** (`Todas` / `Operaron <mes>` / `Operaron <año>`) — la ventana va escrita en el botón porque «operó» sin decir cuándo es la ambigüedad que hacía que dos pantallas dieran distinto: una cuenta que operó en marzo y paró **no** operó este mes y **sí** operó este año (en ANÁLISIS COMERCIAL figura `DORMIDA`, que mide días desde la última op — otra pregunta). ⚠️ **El Desde NO mueve ninguna de las dos ventanas**, ni CTAS OPS: solo el volumen y el arancel. Por eso el año es una pill propia y no «poner Desde=1/1». **Click en una barra de Q1** en modo op. mes / op. año baja ese segmento a Q4 con el MISMO filtro puesto (ver quiénes, no solo cuántos); antiguo toggle en Q4·Clientes (deja las cuentas con ≥1 boleto en el mes del corte — el MISMO predicado que CTAS OPS; el flag `opero_mes` viaja por fila, así filtra sin refetch y no puede quedar desfasado de lo dibujado). ⚠️ **A propósito NO es un filtro de la vista**: si achicara también el universo, el % de Q1 (`operativas / cuentas del segmento`) daría **100 % en todos los segmentos** y dejaría de significar nada — misma regla que el filtro de operación de Profundidad, que toca el numerador y nunca la base; botón `?` de ayuda | Ninguna |
