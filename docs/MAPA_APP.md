...
| **Comisiones FCI** | Lo que cobra ACA Valores por la TENENCIA de fondos, sin planilla: tabla por fondo (**AuM** = valuación en la foto de corte · arancel del día · acumulado del mes; cada fila se abre y muestra las cuentas que la componen), acumulado mensual histórico (barras ARS/USD separadas) y acumulado por sociedad gerente. Fórmula `valuación × fee_admin ÷ 2 ÷ 365` devengada por día CORRIDO (cada foto cubre hasta la siguiente: el viernes devenga 3 días). Un fondo sin `fee_admin` NO vale cero: se marca `sin fee` y se cuenta aparte. Modal `?` con la fórmula y el fee de cada fondo/gerente. **Todo el cálculo vive en `api/services/comisiones_fci.py`; el front no deriva nada.** | `GET /comisiones-fci?mes` · `/comisiones-fci/detalle?mes&unidad` · `/comisiones-fci/serie` · `/comisiones-fci/meses` · `/comisiones-fci/fees` | Mes (solo meses con foto; no hay desde/hasta a propósito: la comisión se liquida por mes) | — (solo lectura) |
