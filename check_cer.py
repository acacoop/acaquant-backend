"""
check_cer.py — Diagnóstico de instrumentos CER.

Para cada ticker_corto en Trading.Curvas (curva=cer) muestra:
...
def main():
    ...
    # 1. Todos los CER de Curvas
    curvas = list(db_trading["Curvas"].find(
        {"curva": "cer"},
        {"_id": 0, "ticker_corto": 1}
    ))
    if not curvas:
        print("No hay instrumentos con curva=cer en Trading.Curvas.")
        return
    ...
    print(f"Instrumentos CER en Curvas: {len(curvas)}\n")
    print(f"{'Ticker':<12}  {'Estado'}")
    print("-" * 50)

    ok = sin_pos = sin_assets = 0
    for c in sorted(curvas, key=lambda x: x.get("ticker_corto") or ""):
        tc = c["ticker_corto"]
