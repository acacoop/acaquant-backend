...
# AV AGENT — el diario del programa de IA  ⟨HISTÓRICO⟩

> **Por qué las cosas quedaron como quedaron.** La especificación viva es `docs/AGENT_2.0.md`.
>
...
>
> (`QUANTAI.md`, `COPILOTO.md` y `TOOLS_IA.md` se borraron.)
>
> **⚠️ ESTE DOC ES SOLO EL DIARIO** (§0.a → §0.do): las decisiones en orden
> cronológico, con su porqué. El mapa del código que vivía arriba se borró el
> 2026-08-30 porque describía el agente viejo. **Cómo funciona hoy:
> `docs/AGENT_2.0.md`.**
>
...
# ⚠️ ACÁ VIVÍA «CÓMO FUNCIONA HOY — el mapa del código»

Ese bloque (§M.0–M.12, 577 líneas) se **borró el 2026-08-30**. Describía el
agente VIEJO —34 services `av_agent_*`, `core/ciclo.py`, 17 tablas y
`src/components/av-agent/`— como si fuera el estado actual, y decía de sí mismo
«**si las dos se contradicen, manda el mapa**». Nada de eso existe desde
AGENT 2.0 (2026-08-24): el agente vive en `agente/` con **4 tablas** y **un**
reloj, y su especificación es **`docs/AGENT_2.0.md`**.

Un doc histórico con un capítulo que se autodeclara vigente es peor que un doc
viejo entero: el que lo lee sigue la parte que se presenta como verdad.

**Lo que sigue —§0.a en adelante— es el DIARIO, y se conserva a propósito:**
cada §0.x es un bug real y la decisión que lo cerró. Sirve para no repetirlos.
Hay ~25 punteros a estas secciones por número desde el código y otros docs.
