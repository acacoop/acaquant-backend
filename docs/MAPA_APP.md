...
- **Archivos front**: `renta-fija-live.tsx` (shell 2×2 + poll unificado), `curvas-tab.tsx` (tab CURVAS: filtro de TIPO de emisor + filtro de EMISOR por nombre + filtro de TEA + las dos columnas), `bonos-table.tsx`, `bono-modal.tsx` (**la FICHA del bono** — layout 50/50: tasas arriba, ficha vertical a la izquierda, gráfico + cronograma a la derecha), `renta-fija-table.tsx`, `libro-panel.tsx`, `curvas-chart.tsx`, `fair-value-view.tsx`, `fair-value-modal.tsx`, `forwards-panel.tsx`, `forward-matrix.tsx`, `forward-matrix-zscore.tsx`, `breakevens-block.tsx`.
